...
fn repeatable_context() -> ExecutionContext {
    ...
    context.inputs.insert(
        "logits".to_string(),
        RuntimeValue::Tensor(std::sync::Arc::new(
            volta::ir::Tensor::new(vec![2], vec![0.5, 2.5]).unwrap(),
        )),
    );
    context.parameters.insert(
        "w".to_string(),
        RuntimeValue::Tensor(std::sync::Arc::new(
            volta::ir::Tensor::new(vec![2, 2], vec![1.0, 0.25, -0.5, 0.75]).unwrap(),
        )),
    );
    context.parameters.insert(
        "b".to_string(),
        RuntimeValue::Tensor(std::sync::Arc::new(
            volta::ir::Tensor::new(vec![1, 2], vec![0.1, -0.1]).unwrap(),
        )),
    );
    context
}
...
fn multi_softmax_context() -> ExecutionContext {
    ...
    context.inputs.insert(
        "logits_a".to_string(),
        RuntimeValue::Tensor(std::sync::Arc::new(
            volta::ir::Tensor::new(vec![2], vec![0.2, 0.8]).unwrap(),
        )),
    );
    context.inputs.insert(
        "logits_b".to_string(),
        RuntimeValue::Tensor(std::sync::Arc::new(
            volta::ir::Tensor::new(vec![2], vec![1.0, 3.0]).unwrap(),
        )),
    );
    context
}

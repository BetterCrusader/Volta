...
pub struct SemanticAnalyzer {
    ...
    structs: HashMap<String, Vec<(String, ValueType)>>,
...
impl SemanticAnalyzer {
    ...
    fn collect_named_entities(&mut self, statements: &[Stmt]) -> Result<(), SemanticError> {
        for stmt in statements {
            match stmt {
                ...
                Stmt::For { body, .. } => self.collect_named_entities(body)?,
                Stmt::If {
                    then_branch,
                    elif_branches,
                    else_branch,
    ...
    fn analyze_stmt(&mut self, stmt: &Stmt) -> Result<(), SemanticError> {
        match stmt {
            ...
            Stmt::Assign { name, value, span } => {
                ...
                let value_type = self.infer_expr_type(value)?;
                if !is_assignable(target_type.clone(), value_type.clone()) {
                    return Err(Self::error(
                        format!(
            ...
            Stmt::For {
                ...
                if start_type != ValueType::Int {
                    return Err(Self::error(
                        "For loop start must be integer".to_string(),
                        start.span(),
                    ));
                }
                if end_type != ValueType::Int {
                    return Err(Self::error(
                        "For loop end must be integer".to_string(),
                        end.span(),
                    ));
                }

                self.push_scope();
                self.declare_variable(var.clone(), ValueType::Int, *span)?;
                let result = self.analyze_statements(body);
                self.pop_scope();
                result?;
            }
    ...
    fn validate_property_value_types(
        ...
        match (context, prop.key.as_str()) {
            ...
            ("model", "use") => {
                self.expect_exact_count(context, prop, 1)?;
                let name = self.extract_symbol(&prop.values[0]).ok_or_else(|| {
                    Self::error(
                        "Property 'use' in model block requires a function name symbol".to_string(),
                        prop.values[0].span(),
                    )
                })?;
                if !self.functions.contains_key(&name) {
                    return Err(Self::error(
                        format!("Undefined function in model 'use': '{name}'"),
                        prop.values[0].span(),
                    ));
                }
            }
            ...
            ("dataset", "type") => {
                self.expect_exact_count(context, prop, 1)?;
                if let Expr::Ident { name, .. } = &prop.values[0] {
                    let valid = ["xor", "synthetic"];
                    if !valid.contains(&name.as_str()) {
                        return Err(Self::error(
                            format!(
                                "Unknown dataset type '{}'. Valid types: {}",
                                name,
                                valid.join(", ")
                            ),
                            prop.values[0].span(),
                    ...
                    return Err(Self::error(
                        "dataset.type expects an identifier (e.g. xor, synthetic)".to_string(),
                        prop.values[0].span(),
                    ));
                }
            }
        ...
        match expr {
            ...
            Expr::Call { callee, args, span } => {
                if let Some(sig) = self.functions.get(callee).cloned() {
                    ...
                    for (arg, (_, expected_ty)) in args.iter().zip(fields.iter()) {
                        let got_ty = self.infer_expr_type(arg)?;
                        if got_ty != *expected_ty {
                            return Err(Self::error(
                                format!(
                                    "Type mismatch in struct '{}' constructor: expected {:?}, got {:?}",
                                    callee, expected_ty, got_ty
                                ),
                                arg.span(),
                            ));
                        }
            ...
            Expr::MemberAccess {
                ...
                if let ValueType::Object { name } = obj_ty {
                    if let Some(fields) = self.structs.get(&name) {
                        if let Some((_, field_ty)) = fields.iter().find(|(n, _)| n == member) {
                            Ok(field_ty.clone())
                        } else {
                            Err(Self::error(
            ...
            Expr::Symbol { .. } => Ok(ValueType::Unknown),
            ...
            Expr::Binary {
                ...
                match op {
                    BinaryOp::Add | BinaryOp::Sub | BinaryOp::Mul | BinaryOp::Div => {
                        ...
                        if !is_numeric(left_ty.clone()) || !is_numeric(right_ty.clone()) {
                            return Err(Self::error(

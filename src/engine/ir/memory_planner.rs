...
pub struct MemoryPlan {
    ...
    pub offsets: HashMap<ValueId, usize>,
}
...
pub fn plan_memory(graph: &Graph) -> Result<MemoryPlan, MemoryPlanError> {
    ...
    // Optimization: identify values that can be freed early or alias
    // For now, we use a simple liveness analysis based on last use.
    ...
    // Greedy offset allocation (Task 6)
    let mut offsets = HashMap::new();
    let mut total_allocation_size = 0usize;
    let mut active_intervals: Vec<&ValueLiveness> = Vec::new();

    // Sort by start node for greedy allocation
    let mut sorted_values = values.clone();
    sorted_values.sort_by_key(|v| v.start_node);

    for val in &sorted_values {
        // Find a free offset by checking overlaps with active intervals
        // This is a simplified version of the "Interval Coloring" problem
            ...
            for active in &active_intervals {
                ...
                if overlap {
                    if let Some(active_offset) = offsets.get(&active.value) {
                        ...
                        if offset < active_end && *active_offset < (offset + val.estimated_bytes) {
                            conflict = true;
                            offset = active_end; // Try next offset
                            break;
                        }
                    }
                }
        ...
        offsets.insert(val.value, offset);
        active_intervals.push(val);
        total_allocation_size = total_allocation_size.max(offset + val.estimated_bytes);
    }
...
mod tests {
    ...
    fn plans_liveness_and_peak_usage() {
        ...
        assert!(plan.total_allocation_size >= 8);
        assert_eq!(plan.offsets.len(), plan.values.len());

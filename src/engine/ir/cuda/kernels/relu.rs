...
pub fn leaky_relu_f32(
    ...
    if !alpha.is_finite() {
        return Err(format!("leaky_relu alpha must be finite, got {alpha}"));
    }
    if input.is_empty() {
        return Ok(Vec::new());
    }
    ...
    let numel = i32::try_from(input.len())
        .map_err(|_| format!("leaky_relu element count does not fit i32: {}", input.len()))?;
    ...
    unsafe {
        stream
            ...
            .arg(&numel)
            .launch(cfg)
            .map_err(|err| format!("CUDA kernel launch failed for leaky_relu: {err}"))?;
    }

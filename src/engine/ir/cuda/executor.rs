...
fn evaluate_node_cuda(
    ...
    match op {
        ...
        Op::Plugin { operator, inputs } => {
            ...
            for input in inputs {
                resolved_inputs.push(read_value(values, *input)?);
            }
            operator
                .execute_cuda(&resolved_inputs, context)
                .map_err(|err| CudaExecutionError {
                    message: format!("Plugin CUDA execution failed: {}", err.message),
                })
        }
        Op::Add(left, right) => {
        ...
        Op::ElementwiseChain { input, ops } => {
            ...
            for op in ops {
                match op {
                    ...
                    ElementwiseUnaryOp::Sigmoid
                    | ElementwiseUnaryOp::Gelu
                    | ElementwiseUnaryOp::GeluExact
                    | ElementwiseUnaryOp::Exp
                    | ElementwiseUnaryOp::Log => {
                        return Err(CudaExecutionError {
                            message: format!("CUDA {:?} kernel is not implemented yet", op),
                        });
                    }
                }
            }
        ...
        Op::ReduceMaxBackward {
            ...
            let (input_shape, input_data) = read_tensor(values, *input)?;
            let (omax_shape, omax_data) = read_tensor(values, *output_max)?;
            let (up_shape, up_data) = read_tensor(values, *upstream)?;

            let input_t = crate::ir::tensor::Tensor::new(input_shape, input_data).unwrap();
            let omax_t = crate::ir::tensor::Tensor::new(omax_shape, omax_data).unwrap();
            let up_t = crate::ir::tensor::Tensor::new(up_shape, up_data).unwrap();

            let out = crate::engine::ir::kernels::reduce::reduce_max_backward(
                &input_t, &omax_t, &up_t, *axis,
            )
            .map_err(|err| CudaExecutionError {
                message: err.message,
            })?;
        ...
        Op::SigmoidBackward(input, grad) => {
            ...
            let out = sigmoid_backward(&x, &g).map_err(|err| CudaExecutionError {
                message: err.message,
            })?;
            Ok(RuntimeValue::Tensor(std::sync::Arc::new(out)))
        }
        ...
        Op::GeluBackward(input, grad) => {
            ...
            let out = gelu_backward(&x, &g).map_err(|err| CudaExecutionError {
                message: err.message,
            })?;
            Ok(RuntimeValue::Tensor(std::sync::Arc::new(out)))
        }
        Op::GeluExactBackward(input, grad) => {
            ...
            let out = gelu_exact_backward(&x, &g).map_err(|err| CudaExecutionError {
                message: err.message,
            })?;
            Ok(RuntimeValue::Tensor(std::sync::Arc::new(out)))
        }
        ...
        Op::BatchNorm {
            ...
            let (_, bias_data) = read_tensor(values, *bias)?;
            let (_, mean_data) = read_tensor(values, *mean)?;
            let (_, var_data) = read_tensor(values, *var)?;
            ...
            .map_err(|msg| CudaExecutionError { message: msg })?;
        ...
        Op::BatchNormBackwardInput {
            ...
            let (shape, _) = read_tensor(values, *input)?;
            if shape.len() != 4 {
                return Err(CudaExecutionError {
                    message: "BatchNormBackwardInput expects rank-4 input".to_string(),
                });
            }
            let (_, up_data) = read_tensor(values, *upstream)?;
            let (_, w_data) = read_tensor(values, *weight)?;
            let (_, v_data) = read_tensor(values, *var)?;

            let out = batch_norm_backward_input_nchw_f32(
                device,
        ...
        Op::BatchNormBackwardWeight {
            ...
            let (shape, input_data) = read_tensor(values, *input)?;
            if shape.len() != 4 {
                return Err(CudaExecutionError {
                    message: "BatchNormBackwardWeight expects rank-4 input".to_string(),
                });
            }
            let (_, up_data) = read_tensor(values, *upstream)?;
            let (_, m_data) = read_tensor(values, *mean)?;
            let (_, v_data) = read_tensor(values, *var)?;

            let out = batch_norm_backward_weight_nchw_f32(
                device,
                &input_data,
                [shape[0], shape[1], shape[2], shape[3]],
                &up_data,
            ...
            Ok(RuntimeValue::Tensor(std::sync::Arc::new(
                crate::ir::tensor::Tensor::new(vec![shape[1]], out).unwrap(),
            )))
        }
        Op::BatchNormBackwardBias { upstream } => {
            let (shape, up_data) = read_tensor(values, *upstream)?;
            if shape.len() != 4 {
                return Err(CudaExecutionError {
                    message: "BatchNormBackwardBias expects rank-4 upstream".to_string(),
                });
            }

            let out = batch_norm_backward_bias_nchw_f32(
                device,
                &up_data,
                [shape[0], shape[1], shape[2], shape[3]],
                determinism,
        ...
        | Op::LayerNormBackwardBias { .. } => Err(CudaExecutionError {
            message: "CUDA LayerNorm execution is not implemented".to_string(),
        }),
        ...
        | Op::MaxPoolBackward { .. }
        | Op::AvgPoolBackward { .. }
        | Op::Identity(_) => Err(CudaExecutionError {
            message: "CUDA execution for this layer is not implemented".to_string(),
        }),
    }
}
...
fn read_tensor(
    ...
    match read_value(values, id)? {
        RuntimeValue::Tensor(tensor) => Ok((tensor.shape.clone(), tensor.data.to_vec())),
        _ => Err(CudaExecutionError {

...
use crate::ir::{Graph, GraphError, block::BasicBlockId, node::ValueId, op::Op};
...
pub fn add_transformer_encoder_block(
    ...
    let ffn1_drop = if config.dropout > 0.0 {
        let (_, d) = graph.add_op(
            block,
            Op::Dropout {
                input: ffn1_act,
                ratio: config.dropout,
            },
        )?;
        d
    } else {

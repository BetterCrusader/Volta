...
pub fn compile_mlp_train_dll(
    ...
    std::fs::write(&c_src, &code)
        .map_err(|e| MlpTrainCodegenError { message: format!("write C source: {e}") })?;
    ...
    let s1 = std::process::Command::new(&clang)
        ...
        .status()
        .map_err(|e| MlpTrainCodegenError { message: format!("clang: {e}") })?;
    if !s1.success() {
        return Err(MlpTrainCodegenError { message: format!("compile failed; see {}", c_src.display()) });
    }

    let omp_lib = find_omp_lib();

    let mut shim_args = vec!["-O3", "-march=native", "-ffast-math", "-funroll-loops"];
    if omp_lib.is_some() { shim_args.push("-fopenmp"); }
    let s2 = std::process::Command::new(&clang)
        .args(&shim_args)
        .arg("-c").arg(&shim_src).arg("-o").arg(&shim_obj)
        .status()
        .map_err(|e| MlpTrainCodegenError { message: format!("shim: {e}") })?;
    if !s2.success() {
        return Err(MlpTrainCodegenError { message: "gemm_shim compile failed".into() });
    }

    let mut cmd = std::process::Command::new(&clang);
    cmd.arg(&c_obj).arg(&shim_obj)
       .arg("-shared").arg("-O3").arg("-march=native")
       .arg("-o").arg(out_dll);
    ...
    {
        for sym in &["volta_train_init","volta_train_step","volta_train_loss",
                     "volta_train_get_params","volta_train_set_params","volta_train_free"] {
    ...
    let s3 = cmd.status()
        .map_err(|e| MlpTrainCodegenError { message: format!("link: {e}") })?;
    if !s3.success() {
        return Err(MlpTrainCodegenError { message: "link failed".into() });
    }
    Ok(())
}
...
fn generate_c_source(
    ...
    let nl = layers.len();
    if nl < 2 {
        return Err(MlpTrainCodegenError { message: "need >= 2 layer sizes".into() });
    }
    let n = nl - 1; // number of weight matrices

    let mut s = String::new();
    s.push_str("#include <stdlib.h>\n#include <string.h>\n#include <math.h>\n#include <stdint.h>\n\n");
    s.push_str("extern void volta_gemm_f32(float*,const float*,const float*,int64_t,int64_t,int64_t);\n");
    ...
    for i in 0..n {
        ...
        s.push_str(&format!("  float *wtb{i}; /* w{i}^T [{c}x{r}] — updated after SGD */\n"));
        s.push_str(&format!("  float *xt{i};  /* act{i}^T [{r}xbatch] — computed after forward */\n"));
    ...
    for i in 0..(n - 1) {
        s.push_str(&format!("  float *pre{i}; /* batch x {} — pre-relu */\n", layers[i + 1]));
    }
    ...
    for i in 0..n {
        let (r, c) = (layers[i], layers[i + 1]);
        s.push_str(&format!("  h->w{i}=(float*)calloc({r}*{c},4); h->b{i}=(float*)calloc({c},4);\n"));
        s.push_str(&format!("  h->dw{i}=(float*)calloc({r}*{c},4); h->db{i}=(float*)calloc({c},4);\n"));
    ...
    for i in 0..=n {
        s.push_str(&format!("  h->act{i}=(float*)calloc(batch*{},4);\n", layers[i]));
    }
    for i in 0..(n - 1) {
        s.push_str(&format!("  h->pre{i}=(float*)calloc(batch*{},4);\n", layers[i + 1]));
    }
    // Xavier init
    if init_weights.is_none() {
        ...
        for i in 0..n {
            let (r, c) = (layers[i], layers[i + 1]);
            s.push_str(&format!("  volta_transpose_f32(h->wtb{i},h->w{i},{r},{c});\n"));
        }
    }
    s.push_str("  return h;\n}\n\n");
    ...
    for i in 0..n { s.push_str(&format!("  wo[{i}]=h->w{i}; bo[{i}]=h->b{i};\n")); }
    s.push_str("}\n\n");
    ...
    for i in 0..n {
        s.push_str(&format!("  free(h->w{i});free(h->b{i});free(h->dw{i});free(h->db{i});\n"));
        s.push_str(&format!("  free(h->wtb{i});free(h->xt{i});free(h->delta{i});\n"));
    }
    for i in 0..=n { s.push_str(&format!("  free(h->act{i});\n")); }
    for i in 0..(n - 1) { s.push_str(&format!("  free(h->pre{i});\n")); }
    s.push_str("  free(h);\n}\n\n");

    // train_step
    s.push_str("void volta_train_step(VoltaTrainHandle* h, const float* X, const float* Y, float lr) {\n");
    ...
    for i in 0..n {
        ...
        s.push_str(&format!("  volta_gemm_f32({dst},h->act{i},h->w{i},(int64_t)B,(int64_t){r},(int64_t){c});\n"));
        // Bias add: loop is vectorised by clang (inner j over contiguous dst/b)
        s.push_str(&format!("  for(int bi=0;bi<B;bi++) for(int j=0;j<{c};j++) {dst}[bi*{c}+j]+=h->b{i}[j];\n"));
        if !is_last {
            // ReLU activation
            s.push_str(&format!("  for(int k=0;k<B*{c};k++) h->act{}[k]=h->pre{i}[k]>0.0f?h->pre{i}[k]:0.0f;\n", i+1));
        }
    ...
    for i in (0..n).rev() {
        ...
        // ReLU backward
        if !is_last {
            s.push_str(&format!("  for(int k=0;k<B*{c};k++) h->delta{i}[k]*=(h->pre{i}[k]>0.0f?1.0f:0.0f);\n"));
        }

        // Compute xt{i} = act{i}^T  [r×B] in backward (act{i} is stable here)
        s.push_str(&format!("  volta_transpose_f32(h->xt{i},h->act{i},(int64_t)B,(int64_t){r});\n"));
        ...
        s.push_str(&format!("  for(int bi=0;bi<B;bi++) for(int j=0;j<{c};j++) h->db{i}[j]+=h->delta{i}[bi*{c}+j];\n"));
        ...
        // SGD update
        let wn = r * c;
        s.push_str(&format!("  for(int k=0;k<{wn};k++) h->w{i}[k]-=lr*h->dw{i}[k];\n"));
        s.push_str(&format!("  for(int k=0;k<{c};k++) h->b{i}[k]-=lr*h->db{i}[k];\n"));
    ...
    if let Some(weights) = init_weights {
        ...
        for i in 0..n {
            ...
            if let Some(wd) = weights.get(&wname) {
                let vals: String = wd.iter().map(|v| format!("{v:.8}f")).collect::<Vec<_>>().join(",");
            ...
            if let Some(bd) = weights.get(&bname) {
                let vals: String = bd.iter().map(|v| format!("{v:.8}f")).collect::<Vec<_>>().join(",");
                s.push_str(&format!("  {{static const float _b[]={{{vals}}};memcpy(h->b{i},_b,{c}*4);}}\n"));
            }
        }
        s.push_str("}\n");
    }

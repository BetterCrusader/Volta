...
fn fold_binary(
    ...
    let left_const = constants.get(left.0).and_then(|value| value.as_ref())?;
    let right_const = constants.get(right.0).and_then(|value| value.as_ref())?;
    let folded = fold(left_const.clone(), right_const.clone())?;
...
fn fold_add(left: ConstValue, right: ConstValue) -> Option<ConstValue> {
    match (left, right) {
        ...
        (ConstValue::Tensor(a), ConstValue::Tensor(b)) => a.add(&b).ok().map(ConstValue::Tensor),
        _ => None,
    }
}
...
fn fold_sub(left: ConstValue, right: ConstValue) -> Option<ConstValue> {
    match (left, right) {
        ...
        (ConstValue::Tensor(a), ConstValue::Tensor(b)) => a.sub(&b).ok().map(ConstValue::Tensor),
        _ => None,
    }
}
...
fn op_from_const(value: ConstValue) -> Op {
    match value {
        ...
        ConstValue::Tensor(tensor) => Op::ConstTensor {

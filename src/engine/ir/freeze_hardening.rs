...
mod tests {
    ...
    #[test]
    fn pass_order_matrix_is_deterministic_per_order() {
        let base = build_fuzz_graph(2026, 32);
        let orders = vec![
            vec![0, 1, 2, 3, 4, 5, 6, 7],
            vec![7, 6, 5, 4, 3, 2, 1, 0],
            vec![1, 3, 5, 7, 0, 2, 4, 6],
        ...
        for order in orders {
            ...
            verify_graph(&g1).expect("g1 must verify");
            verify_graph(&g2).expect("g2 must verify");

            let fp1 = graph_fingerprint(&g1);
            let fp2 = graph_fingerprint(&g2);
            assert_eq!(fp1, fp2, "fingerprint must be stable for pass order");

            let p1 = build_execution_plan(&g1, &HashSet::new()).expect("plan g1");
            let p2 = build_execution_plan(&g2, &HashSet::new()).expect("plan g2");
            assert_eq!(
                crate::ir::schedule_hash(&p1.schedule),
                crate::ir::schedule_hash(&p2.schedule),
                "schedule hash must be stable for pass order"
            );
            assert_eq!(p1.allocation.peak_bytes, p2.allocation.peak_bytes);
        }
    }
    ...
    fn deterministic_training_repeats_match() {
        ...
        let cfg = TrainConfig::new(8, OptimizerConfig::Sgd { lr: 0.01 });
    ...
    fn long_run_determinism_100x50_heavy() {
        let start = std::time::Instant::now();
        let (graph, loss, params, dataset) = train_fixture();
        let cfg = TrainConfig::new(100, OptimizerConfig::Sgd { lr: 0.005 });

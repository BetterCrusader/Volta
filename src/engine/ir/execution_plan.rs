...
pub struct ExecutionPlan {
    ...
    /// Set to true when the plan was built by `build_execution_plan`, which
    /// already ran all three verification passes. The runtime hot-path can
    /// skip redundant re-verification when this flag is set.
    pub verified: bool,
    /// Cached determinism level and compiled fingerprint, set after the first
    /// successful `compile_or_get_cached` call. Allows subsequent execute
    /// calls to bypass env-var reads and Mutex locking entirely.
    pub cached_determinism: Option<crate::ir::DeterminismLevel>,
}
...
pub fn build_execution_plan(
    ...
    // Run optimization passes only for inference (no gradient tracking).
    // During training the forward graph is shared with autograd — modifying
    // ValueIds would invalidate backward references.
    let opt_graph;
    let graph = if gradient_values.is_empty() {
        opt_graph = {
            let mut g = graph.clone();
            let mut pm = PassManager::new();
            pm.add_pass(Box::new(ConstantFoldingPass::new()));
            pm.add_pass(Box::new(AlgebraicSimplificationPass::new()));
            pm.add_pass(Box::new(CsePass::new()));
            pm.add_pass(Box::new(DcePass::new()));
            pm.add_pass(Box::new(ElementwiseFusionPass::new()));
            pm.run(&mut g);
            g
        };
        &opt_graph

...
use crate::engine::ir::kernels::utils::{ensure_same_shape, product_prefix, product_suffix, should_par};
...
pub fn add(left: &Tensor, right: &Tensor) -> Result<Tensor, TensorError> {
    ...
    let out: Vec<f32> = {
        ...
        if should_par(left_c.data.len()) {
            left_c.data.par_iter().zip(right_c.data.par_iter()).map(|(a, b)| a + b).collect()
        } else {
            left_c.data.iter().zip(right_c.data.iter()).map(|(a, b)| a + b).collect()
        }
...
pub fn matmul(left: &Tensor, right: &Tensor) -> Result<Tensor, TensorError> {
    if left.shape.len() != 2 || right.shape.len() != 2 {
        return Err(TensorError { message: "Matmul expects rank-2".to_string() });
    }
    let (m, k, n) = (left.shape[0], left.shape[1], right.shape[1]);
    if k != right.shape[0] {
        return Err(TensorError { message: format!("Shape mismatch in matmul: [{m}x{k}] @ [{k_right}x{n}]", k_right = right.shape[0]) });
    }
...
pub fn concat(tensors: &[Tensor], axis: usize) -> Result<Tensor, TensorError> {
    if tensors.is_empty() { return Err(TensorError { message: "Empty list in concat".to_string() }); }
    ...
    let (outer, inner) = (product_prefix(&out_shape, axis)?, product_suffix(&out_shape, axis + 1)?);
...
pub fn gather(tensor: &Tensor, indices: &[usize], axis: usize) -> Result<Tensor, TensorError> {
    ...
    let (outer, inner) = (product_prefix(&contig.shape, axis)?, product_suffix(&contig.shape, axis + 1)?);
    let total_elements: usize = out_shape.iter().product();
    let mut out = Vec::with_capacity(total_elements);
    for o in 0..outer {
        for &idx in indices {
...
pub fn log_elementwise(tensor: &Tensor) -> Result<Tensor, TensorError> {
    ...
    let out: Vec<f32> = {
        let f = |&v: &f32| -> Result<f32, TensorError> {
            if v <= 0.0 { Err(TensorError { message: format!("log of non-positive value: {v}") }) } else { Ok(v.ln()) }
        };
        #[cfg(feature = "parallel")]
...
pub fn exp_elementwise(tensor: &Tensor) -> Result<Tensor, TensorError> {
    ...
    let out: Vec<f32> = {
        let f = |&v: &f32| -> Result<f32, TensorError> {
            let e = v.exp();
            if e.is_finite() { Ok(e) } else { Err(TensorError { message: format!("exp overflow for value: {v}") }) }
        };
        #[cfg(feature = "parallel")]
...
pub fn embedding(weight: &Tensor, indices: &Tensor) -> Result<Tensor, TensorError> {
    ...
    if w.shape.len() != 2 {
        return Err(TensorError { message: "Embedding weight must be rank-2 [vocab_size, embed_dim]".to_string() });
    }
    ...
    for (t, &raw_idx) in idx.data.iter().enumerate() {
        ...
        if i >= vocab_size {
            return Err(TensorError { message: format!("Embedding index {i} out of vocab_size {vocab_size}") });
        }
        out[t * embed_dim..(t + 1) * embed_dim].copy_from_slice(&w.data[i * embed_dim..(i + 1) * embed_dim]);
    }
...
pub fn embedding_backward(weight: &Tensor, indices: &Tensor, upstream: &Tensor) -> Result<Tensor, TensorError> {
    let w = weight.make_contiguous()?;
    let idx = indices.make_contiguous()?;
    let up = upstream.make_contiguous()?;
    if w.shape.len() != 2 {
        return Err(TensorError { message: "Embedding weight must be rank-2".to_string() });
    }
    let (vocab_size, embed_dim) = (w.shape[0], w.shape[1]);
    let num_tokens = idx.data.len();
    if up.data.len() != num_tokens * embed_dim {
        return Err(TensorError { message: format!(
            "EmbeddingBackward upstream size mismatch: expected {} got {}", num_tokens * embed_dim, up.data.len()
        )});
    }

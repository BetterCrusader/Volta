...
pub fn broadcast_shape(left: &[usize], right: &[usize], op: &str) -> Result<Vec<usize>, TensorError> {
    let max_len = left.len().max(right.len());
    let mut out = vec![0; max_len];
    for i in 0..max_len {
        let l = if i < max_len - left.len() { 1 } else { left[i - (max_len - left.len())] };
        let r = if i < max_len - right.len() { 1 } else { right[i - (max_len - right.len())] };
        out[i] = if l == r { l } else if l == 1 { r } else if r == 1 { l } else {
            return Err(TensorError { message: format!("Cannot broadcast {left:?} and {right:?} in {op}") });
        };
    }
    Ok(out)
}

pub fn broadcast_index(linear: usize, out_shape: &[usize], out_strides: &[usize], in_shape: &[usize], in_strides: &[usize]) -> usize {
    ...
    for (axis, &stride) in out_strides.iter().enumerate() {
        ...
        if stride != 0 { rem %= stride; }
        if axis >= offset {
            let in_axis = axis - offset;
            if in_shape[in_axis] != 1 { input_linear += coord * in_strides[in_axis]; }
        }
    }
    input_linear
}

pub fn elementwise_broadcast_binary<F>(left: &Tensor, right: &Tensor, op: &str, f: F) -> Result<Tensor, TensorError>
where F: Fn(f32, f32) -> Result<f32, TensorError> + Sync + Send {
    ...
    if count == 0 { return Tensor::new(out_shape, vec![]); }
...
pub fn default_strides(shape: &[usize]) -> Vec<usize> {
    ...
    let mut acc = 1;
    for i in (0..shape.len()).rev() { strides[i] = acc; acc *= shape[i]; }
    strides
}

pub fn reduce_keepdims_shape(shape: &[usize], axis: Option<usize>) -> Result<Vec<usize>, TensorError> {
    match axis {
        Some(a) => { if a >= shape.len() { return Err(TensorError { message: "out of bounds".to_string() }); } let mut out = shape.to_vec(); out[a] = 1; Ok(out) }
        None => Ok(if shape.is_empty() { vec![1] } else { vec![1; shape.len()] })
    }
}

pub fn ensure_same_shape(a: &[usize], b: &[usize], op: &str) -> Result<(), TensorError> {
    if a == b { Ok(()) } else { Err(TensorError { message: format!("Shape mismatch in {op}: {a:?} vs {b:?}") }) }
}

pub fn element_count(shape: &[usize]) -> Option<usize> { shape.iter().try_fold(1usize, |acc, &d| acc.checked_mul(d)) }

pub fn product_prefix(shape: &[usize], end: usize) -> Result<usize, TensorError> {
    shape.iter().take(end).try_fold(1usize, |acc, &d| acc.checked_mul(d)).ok_or_else(|| TensorError { message: "overflow".to_string() })
}

pub fn product_suffix(shape: &[usize], start: usize) -> Result<usize, TensorError> {
    shape.iter().skip(start).try_fold(1usize, |acc, &d| acc.checked_mul(d)).ok_or_else(|| TensorError { message: "overflow".to_string() })
}

pub fn erf_approx(x: f32) -> f32 {
    const A: [f32; 5] = [0.254_829_6, -0.284_496_72, 1.421_413_8, -1.453_152_1, 1.061_405_4];

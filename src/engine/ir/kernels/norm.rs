...
pub fn layer_norm(
    ...
    if input_c.shape.len() != 2 {
        return Err(TensorError {
            message: "Layer_norm expects [N, D]".to_string(),
        });
    }
    let (n, d) = (input_c.shape[0], input_c.shape[1]);
    if d == 0 {
        return Err(TensorError {
            message: "D must be > 0".to_string(),
        });
    }
    if w.shape != vec![d] || b.shape != vec![d] {
        return Err(TensorError {
            message: "Weight/bias mismatch in layer_norm".to_string(),
        });
    }
...
pub fn batch_norm_nchw(
    ...
    if input.shape.len() != 4 {
        return Err(TensorError {
            message: format!("BatchNorm expects rank-4 NCHW input, got {:?}", input.shape),
        });
    }
    let input_c = input.make_contiguous()?;
    ...
    if weight_c.shape != vec![c]
        || bias_c.shape != vec![c]
        || mean_c.shape != vec![c]
        || var_c.shape != vec![c]
    {
        return Err(TensorError {
            message: "BatchNorm weight/bias/mean/var shape mismatch".to_string(),
        });
    }
...
pub fn batch_norm_backward_input_nchw(
    ...
    if input.shape.len() != 4 {
        return Err(TensorError {
            message: "BatchNormBackwardInput expects rank-4".to_string(),
        });
    }
    if upstream.shape != input.shape {
        return Err(TensorError {
            message: "Shape mismatch in BatchNormBackwardInput".to_string(),
        });
    }
    let input_c = input.make_contiguous()?;
...
pub fn batch_norm_backward_weight_nchw(
    ...
    if input.shape.len() != 4 {
        return Err(TensorError {
            message: "BatchNormBackwardWeight expects rank-4".to_string(),
        });
    }
    if upstream.shape != input.shape {
        return Err(TensorError {
            message: "Shape mismatch in BatchNormBackwardWeight".to_string(),
        });
    }
    let input_c = input.make_contiguous()?;
    let upstream_c = upstream.make_contiguous()?;
...
pub fn batch_norm_backward_bias_nchw(upstream: &Tensor) -> Result<Tensor, TensorError> {
    if upstream.shape.len() != 4 {
        return Err(TensorError {
            message: "BatchNormBackwardBias expects rank-4".to_string(),
        });
    }
    let upstream_c = upstream.make_contiguous()?;
    let (n, c, h, w) = (
...
pub fn layer_norm_backward_input(
    ...
    let w = weight.make_contiguous()?;
    if input_c.shape.len() != 2 || input_c.shape != dy_c.shape {
        return Err(TensorError {
            message: "Shape mismatch in layer_norm_backward_input".to_string(),
        });
    }
    let (n, d) = (input_c.shape[0], input_c.shape[1]);
    ...
    for i in 0..n {
        ...
        let var: f64 = x_row
            ...
            * inv_d;
        let inv_std = 1.0_f64 / (var + epsilon as f64).sqrt();
        let g: Vec<f64> = dy_row
            .iter()
            .zip(w.data.iter())
            .map(|(&dy_j, &gam_j)| dy_j as f64 * gam_j as f64)
            .collect();
        let x_hat: Vec<f64> = x_row
            .iter()
            .map(|&x_j| (x_j as f64 - mean) * inv_std)
            .collect();
        let sum_g: f64 = g.iter().sum();
        let sum_g_x_hat: f64 = g
            .iter()
            .zip(x_hat.iter())
            .map(|(g_j, xh_j)| g_j * xh_j)
            .sum();
        for j in 0..d {
            out[i * d + j] =
                (inv_d * inv_std * (d as f64 * g[j] - sum_g - x_hat[j] * sum_g_x_hat)) as f32;
        }
    }
    Tensor::new(input_c.shape.clone(), out)
}

pub fn layer_norm_backward_weight(
    input: &Tensor,
    dy: &Tensor,
    epsilon: f32,
) -> Result<Tensor, TensorError> {
    ...
    for i in 0..n {
        ...
        let inv_std = 1.0_f64 / (var + epsilon as f64).sqrt();
        for j in 0..d {
            d_gamma[j] += dy_row[j] as f64 * ((x_row[j] as f64 - mean) * inv_std);
        }
    }
...
pub fn group_norm(
    ...
    if inp.shape.len() < 2 {
        return Err(TensorError {
            message: "GroupNorm expects rank >= 2".to_string(),
        });
    }
    let n = inp.shape[0];
    let c = inp.shape[1];
    if c % num_groups != 0 {
        return Err(TensorError {
            message: format!("GroupNorm: C={c} not divisible by num_groups={num_groups}"),
        });
    }
    if w.shape != vec![c] || b.shape != vec![c] {
        return Err(TensorError {
            message: "GroupNorm weight/bias must have shape [C]".to_string(),
        });
    }
...
pub fn group_norm_backward_input(
    ...
    if inp.shape.len() < 2 || inp.shape != dy_c.shape {
        return Err(TensorError {
            message: "Shape mismatch in group_norm_backward_input".to_string(),
        });
    }
    let n = inp.shape[0];
    let c = inp.shape[1];
        ...
        for g in 0..num_groups {
            ...
            for ci in 0..channels_per_group {
                ...
                for s in 0..spatial {
                    ...
                    let dg = dy_c.data[base + s] as f64 * gamma;
                    grad[base + s] =
                        (inv_std * (dg - inv_m * sum_dg - x_hat * inv_m * sum_dg_xhat)) as f32;
                }
            }
        }
    }
...
pub fn group_norm_backward_weight(
    input: &Tensor,
    dy: &Tensor,
    num_groups: usize,
    epsilon: f32,
    ...
    if inp.shape.len() < 2 || inp.shape != dy_c.shape {
        return Err(TensorError {
            message: "Shape mismatch in group_norm_backward_weight".to_string(),
        });
    }
    let n = inp.shape[0];
    let c = inp.shape[1];
...
pub fn group_norm_backward_bias(dy: &Tensor) -> Result<Tensor, TensorError> {
    ...
    if dy_c.shape.len() < 2 {
        return Err(TensorError {
            message: "GroupNorm backward bias expects rank >= 2".to_string(),
        });
    }
    let n = dy_c.shape[0];
    let c = dy_c.shape[1];
    ...
    for ni in 0..n {
        for ch in 0..c {
            let base = (ni * c + ch) * spatial;
            for s in 0..spatial {
                d_beta[ch] += dy_c.data[base + s] as f64;
            }
        }
    }
...
pub fn instance_norm(
    ...
    let c = if input.shape.len() >= 2 {
        input.shape[1]
    } else {
        return Err(TensorError {
            message: "InstanceNorm expects rank >= 2".to_string(),
        });
    };
...
pub fn instance_norm_backward_input(
    input: &Tensor,
    dy: &Tensor,
    weight: &Tensor,
    epsilon: f32,
...
pub fn instance_norm_backward_weight(
    input: &Tensor,
    dy: &Tensor,
    epsilon: f32,
) -> Result<Tensor, TensorError> {

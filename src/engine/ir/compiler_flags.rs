...
static CACHED_FLAGS: OnceLock<CompilerFlags> = OnceLock::new();

impl CompilerFlags {
    /// Read flags from environment variables once, then return the cached value.
    /// This avoids repeated `std::env::var` system calls in the hot training loop.
    #[must_use]
    pub fn from_env() -> Self {
        *CACHED_FLAGS.get_or_init(|| Self {

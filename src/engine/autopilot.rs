...
impl AutopilotEngine {
    ...
    pub fn resolve(&self, input: &AutopilotContext) -> ResolvedTrainConfig {
        ...
        let precision = if let Some(explicit) = input.train.precision.as_ref() {
            ...
            let selected = if is_gpu_device(&device) {
                "fp16"
            } else {
                "fp32"
            };
...
#[must_use]
fn is_gpu_device(device: &str) -> bool {
    device.eq_ignore_ascii_case("gpu") || device.eq_ignore_ascii_case("cuda")
}
...
mod tests {
    ...
    #[test]
    fn cuda_device_uses_gpu_precision_rule() {
        let mut input = base_context();
        input.train.device = Some("cuda".to_string());

        let engine = AutopilotEngine::new();
        let resolved = engine.resolve(&input);

        assert_eq!(resolved.device, "cuda");
        assert_eq!(resolved.precision, "fp16");
    }
}

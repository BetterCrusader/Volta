...
pub fn hunt_and_extract(model_name: &str) -> Result<(), String> {
    ...
    let found_path = find_model_file(model_name).ok_or_else(|| format!("Model '{}' not found on the system. Check the name.", model_name))?;

    println!("🎯 Target acquired in {:.2}s: {}", start_time.elapsed().as_secs_f64(), found_path.display());
...
fn extract_gguf(path: &Path, model_name: &str) -> Result<(), String> {
    ...
    let gguf_opt = gguf::GGUFFile::read(buffer).map_err(|e| format!("Failed to parse GGUF: {}", e))?;
    ...
    if layer_count == 0 {
        for meta in &gguf.header.metadata {
            if meta.key.contains("block_count") {
                if let gguf::GGUFMetadataValue::Uint32(n) = &meta.value { layer_count = *n as usize; }
            }
        }
    }

    emit_vt_file(model_name, path, model_type, total_params, layer_count, hidden_size, vocab_size)
}

fn extract_safetensors(path: &Path, model_name: &str) -> Result<(), String> {
    let mut file = File::open(path).map_err(|e| format!("Could not open file: {}", e))?;
    let mut len_bytes = [0u8; 8];
    file.read_exact(&mut len_bytes).map_err(|_| "Failed to read header length")?;
    let header_len = u64::from_le_bytes(len_bytes) as usize;

    if header_len > 100_000_000 {
        return Err(format!("Header is suspiciously large ({} bytes).", header_len));
    }

    let mut header_bytes = vec![0u8; header_len];
    file.read_exact(&mut header_bytes).map_err(|_| "Failed to read JSON header")?;
    let metadata: Value = serde_json::from_str(&String::from_utf8_lossy(&header_bytes)).map_err(|_| "Invalid JSON format")?;
    ...
    let safe_model_type = if actual_model_type == "UnknownModel" { "transformer" } else { actual_model_type };

    emit_vt_file(model_name, path, safe_model_type, total_params, layer_count, hidden_size, vocab_size)
}

fn emit_vt_file(model_name: &str, path: &Path, model_type: &str, total_params: u64, layer_count: usize, hidden_size: usize, vocab_size: usize) -> Result<(), String> {
    let clean_name = model_name.replace("-", "_").replace(".safetensors", "").replace(".gguf", "");
...
",
    ...
    let mut out_file = File::create(&output_file_name).map_err(|e| format!("Failed to create file: {}", e))?;
    out_file.write_all(vt_code.as_bytes()).map_err(|e| format!("Failed to write file: {}", e))?;

    println!("✅ Extraction Complete! Architecture saved to 📄 {}", output_file_name);
...
fn find_model_file(name: &str) -> Option<PathBuf> {
    ...
    let search_name = if !name.ends_with(".safetensors") && !name.ends_with(".gguf") && !name.ends_with(".bin") {
            ...
            if path.is_file()
                && let Some(file_name) = path.file_name().and_then(|n| n.to_str()) {
                    if file_name.eq_ignore_ascii_case(&search_name) ||
                       (file_name.ends_with(".safetensors") && file_name.to_lowercase().contains(&name.to_lowercase())) ||
                       (file_name.ends_with(".gguf") && file_name.to_lowercase().contains(&name.to_lowercase())) {

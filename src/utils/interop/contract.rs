...
impl IrGraphContract {
    ...
    pub fn compile(&self) -> Result<ImportedProgram, InteropError> {
        self.compile_with_plugins(None)
    }

    /// Compiles the validated IR graph into an [`ImportedProgram`], using the
    /// provided [`PluginRegistry`] to resolve custom operators.
    ///
    /// # Errors
    ///
    /// Returns `Err(InteropError)` if validation fails, any node cannot be
    /// lowered, or a required plugin operator is missing.
    pub fn compile_with_plugins(
        &self,
        plugins: Option<&crate::interop::PluginRegistry>,
        ...
        for node in &self.nodes {
            let value = match &node.op {
                ...
                IrOpContract::Flatten { input, axis } => {
                    let input = resolve_value(&ids, input)?;
                    let (_, value) = graph
                        .add_op(block, Op::Flatten { input, axis: *axis })
                        .map_err(|err| InteropError::new(err.message))?;
                    value
                }
                ...
                IrOpContract::Identity { input } => {
                    let input = resolve_value(&ids, input)?;
                    let (_, value) = graph
                        .add_op(block, Op::Identity(input))
                        .map_err(|err| InteropError::new(err.message))?;
                    value
                }
                IrOpContract::Plugin { op_name, inputs } => {
                    let mut resolved = Vec::with_capacity(inputs.len());
                    for input in inputs {
                        resolved.push(resolve_value(&ids, input)?);
                    }
                    let operator = plugins
                        .and_then(|reg| reg.create_operator(op_name))
                        .ok_or_else(|| {
                            InteropError::new(format!(
                                "unsupported plugin operator '{op_name}' (no registry or plugin found)"
                            ))
                        })?;
                    let (_, value) = graph
                        .add_op(
                ...
                IrOpContract::BatchNorm {
                    ...
                    let input = resolve_value(&ids, input)?;
                    let weight = resolve_value(&ids, weight)?;
                    let bias = resolve_value(&ids, bias)?;
                    let mean = resolve_value(&ids, mean)?;
                    let var = resolve_value(&ids, var)?;
                    let (_, value) = graph
                        .add_op(
                            block,
                            Op::BatchNorm {
...
fn op_inputs(op: &IrOpContract) -> Vec<&str> {
    match op {
        ...
        IrOpContract::Plugin { inputs, .. } => inputs.iter().map(String::as_str).collect(),
        IrOpContract::BatchNorm {
            input,
            weight,
            bias,
...
mod tests {
    ...
    #[test]
    fn contract_compile_supports_pool_and_flatten_ops() {
        let contract = IrGraphContract {
            version: IrContractVersion::V1,
            name: "pool_flatten".to_string(),
            inputs: vec![IrTensorSpec {
                name: "x".to_string(),
                shape: vec![1, 1, 4, 4],
                dtype: IrDataType::F32,
        ...
        let imported = contract.compile().expect("contract compile should pass");
        assert!(!imported.graph.nodes.is_empty());
    }

    #[test]
    fn contract_compile_supports_batchnorm_op() {
        let contract = IrGraphContract {
            version: IrContractVersion::V1,
            ...
            nodes: vec![
                ...
                IrNodeContract {
                    id: "bn".to_string(),
                    op: IrOpContract::BatchNorm {
                        input: "x".to_string(),
                        weight: "gamma".to_string(),
                        bias: "beta".to_string(),
                        mean: "mean".to_string(),
                        var: "var".to_string(),
                    },
        ...
        let imported = contract
            .compile()
            .expect("batchnorm contract compile should pass");
        assert_eq!(imported.parameter_defaults.len(), 2);
    }
}

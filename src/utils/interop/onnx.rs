...
impl OnnxImporter {
    ...
    pub fn import_graph(&self, source: &OnnxGraphStub) -> Result<ImportedProgram, InteropError> {
        ...
        for node in &source.nodes {
            let ir_op = match &node.op {
                ...
                OnnxOpStub::Plugin { op_name, inputs } => IrOpContract::Plugin {
        ...
        .compile_with_plugins(Some(&self.plugins))
    }
    ...
    pub fn import_model_proto(
        ...
        validate_supported_opset(model)?;
        let graph = model
            .graph
            .as_ref()
...
fn has_attribute(node: &pb::NodeProto, key: &str) -> bool {
    node.attribute.iter().any(|attr| attr.name == key)
}

fn validate_supported_opset(model: &pb::ModelProto) -> Result<(), InteropError> {
    for opset in &model.opset_import {
        let domain = opset.domain.as_str();
        if (domain.is_empty() || domain == "ai.onnx") && opset.version > MAX_SUPPORTED_ONNX_OPSET {
            return Err(InteropError::unsupported_opset(
                opset.version,
...
fn map_proto_op(
    ...
    Ok(match op {
        ...
        "Conv" => {
            ...
            if let Some(group) = attribute_i64(node, "group")
                && group != 1
            {
                return Err(InteropError::invalid_attribute(
                    &node.name,
                    &node.op_type,
                    "group",
            ...
            if let Some(dilations) = attribute_i64s(node, "dilations")
                && dilations != vec![1, 1]
            {
                return Err(InteropError::invalid_attribute(
                    &node.name,
                    &node.op_type,
                    "dilations",
                    "only dilations=[1,1] is currently supported",
                ));
            }
            if let Some(strides) = attribute_i64s(node, "strides")
                && strides != vec![1, 1]
            {
                return Err(InteropError::invalid_attribute(
                    &node.name,
                    &node.op_type,
                    "strides",
                    "only strides=[1,1] is currently supported",
                ));
            }
            if let Some(pads) = attribute_i64s(node, "pads")
                && pads.iter().any(|pad| *pad != 0)
            {
                return Err(InteropError::invalid_attribute(
                    &node.name,
                    &node.op_type,
                    "pads",
                    "only zero pads are currently supported",
                ));
            }
            if has_attribute(node, "auto_pad") {
                return Err(InteropError::invalid_attribute(
                    &node.name,
                    &node.op_type,
                    "auto_pad",
                    "auto_pad is not supported in static Wave 2 import",
                ));
            }
        ...
        unsupported => IrOpContract::Plugin {
            op_name: unsupported.to_string(),
...
fn op_kind(op: &OnnxOpStub) -> &'static str {
    match op {
        ...
        OnnxOpStub::Plugin { op_name, .. } => Box::leak(op_name.clone().into_boxed_str()),
...
fn op_kind_from_contract(op: &IrOpContract) -> &'static str {
    match op {
        ...
        IrOpContract::Plugin { op_name, .. } => Box::leak(op_name.clone().into_boxed_str()),
...
fn inferred_output_rank(op: &IrOpContract, value_ranks: &HashMap<String, usize>) -> Option<usize> {
    match op {
        ...
        IrOpContract::Plugin { .. } => None,
    }
}

#[cfg(test)]
mod tests {
    use super::{map_proto_op, validate_supported_opset};
    use std::collections::HashMap;
    use tract_onnx::pb;

    #[test]
    fn maps_unsupported_op_to_plugin() {
        let node = pb::NodeProto {
            name: "n0".to_string(),
            op_type: "UnknownFancyOp".to_string(),
            input: vec!["x".to_string()],
            ..Default::default()
        };
        let op = map_proto_op(&node, &HashMap::new(), &HashMap::new())
            .expect("should now map to plugin");
        match op {
            super::IrOpContract::Plugin { op_name, inputs } => {
                assert_eq!(op_name, "UnknownFancyOp");
                assert_eq!(inputs, vec!["x"]);
            }
            _ => panic!("Expected Plugin op, got {:?}", op),
        }
    }
    ...
    fn executes_imported_plugin_on_cpu() {
        ...
        if let RuntimeValue::Int(v) = result {
            assert_eq!(v, 11);
        } else {
            panic!("Expected Int(11), got {:?}", result);
        }
    }

    #[test]
    fn rejects_conv_with_unsupported_stride_attribute() {
        let node = pb::NodeProto {
        ...
        let err = map_proto_op(&node, &HashMap::new(), &HashMap::new())
            .expect_err("unsupported strides must fail");
        assert!(err.message.contains("[INVALID_ATTRIBUTE]"));
        assert!(err.message.contains("strides"));
    }

    #[test]
    fn rejects_model_with_too_new_opset() {
        ...
        let err = validate_supported_opset(&model).expect_err("opset must be rejected");
        assert!(err.message.contains("[UNSUPPORTED_OPSET]"));
    }
}

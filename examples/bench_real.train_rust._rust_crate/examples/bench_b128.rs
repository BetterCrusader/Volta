...
impl State {
    fn new(batch: usize) -> Self {
        let mut rng = 42u64;
        let mut ws: [Vec<f32>; NL] = std::array::from_fn(|i| vec![0.0f32; LAYERS[i]*LAYERS[i+1]]);
...
fn bench(label: &str, steps: usize, runs: usize, mut f: impl FnMut()) -> f64 {
    ...
    println!("  {label:40} median={med:.3} ms/step  all={}",
        results.iter().map(|x| format!("{:.3}",x)).collect::<Vec<_>>().join(","));
    med
}
...
fn main() {
    ...
    let xa: Vec<f32> = (0..128*LAYERS[0]).map(|i| (i%17) as f32 * 0.01 - 0.08).collect();
    let ya: Vec<f32> = (0..128*LAYERS[NL]).map(|i| (i%7) as f32 * 0.1).collect();
    let lr = 0.01f32;

...
#[target_feature(enable = "avx512f")]
#[allow(dead_code)]
unsafe fn sgemm_m64(c: *mut f32, a: *const f32, b: *const f32, m: usize, k: usize, n: usize) {
    // Zero output first
...
#[cfg(target_arch = "x86_64")]
#[target_feature(enable = "avx512f")]
unsafe fn sgemm_m64_tiled(c: *mut f32, a: *const f32, b: *const f32, m: usize, k: usize, n: usize) {
    use std::arch::x86_64::*;
    // Zero output
    std::ptr::write_bytes(c, 0, m * n);
        ...
        if jj < n {
            ...
            for i in 0..m {
                ...
                for kp in kk..ke {
                    ...
                    acc = _mm512_fmadd_ps(
                        a_val,
                        _mm512_maskz_loadu_ps(mask, b.add(kp * n + jj)),
                        acc,
                    );
                }
                let cp = c.add(i * n + jj);
                _mm512_mask_storeu_ps(
                    cp,
                    mask,
                    _mm512_add_ps(_mm512_maskz_loadu_ps(mask, cp), acc),
                );
            }
        }
        kk += KT;
    ...
    for rr in 0..r {
        ...
        while cc + CT <= c {
            ...
            while mi + MT <= m {
                for ii in mi..mi + MT {
                    ...
                    acc1 =
                        _mm512_fmadd_ps(a_val, _mm512_loadu_ps(delta.add(ii * c + cc + 16)), acc1);
                    acc2 =
                        _mm512_fmadd_ps(a_val, _mm512_loadu_ps(delta.add(ii * c + cc + 32)), acc2);
                    acc3 =
                        _mm512_fmadd_ps(a_val, _mm512_loadu_ps(delta.add(ii * c + cc + 48)), acc3);
                }
                mi += MT;
            }
            for ii in mi..m {
            ...
            _mm512_storeu_ps(
                wp.add(16),
                _mm512_fmadd_ps(neg_lr, acc1, _mm512_loadu_ps(wp.add(16))),
            );
            _mm512_storeu_ps(
                wp.add(32),
                _mm512_fmadd_ps(neg_lr, acc2, _mm512_loadu_ps(wp.add(32))),
            );
            _mm512_storeu_ps(
                wp.add(48),
                _mm512_fmadd_ps(neg_lr, acc3, _mm512_loadu_ps(wp.add(48))),
            );
            cc += CT;
        }
        // remainder C
        ...
        if cc < c {
            ...
            for ii in 0..m {
                ...
                acc = _mm512_fmadd_ps(
                    a_val,
                    _mm512_maskz_loadu_ps(mask, delta.add(ii * c + cc)),
                    acc,
                );
            }
            let wp = w.add(rr * c + cc);
            _mm512_mask_storeu_ps(
                wp,
                mask,
                _mm512_fmadd_ps(neg_lr, acc, _mm512_maskz_loadu_ps(mask, wp)),
            );
        }
    }
}
...
fn main() {
    ...
    for &(k, n) in &[
        ...
        let t_ref = bench_fn(
            &format!("gemm_crate [{}×{}]@[{}×{}]", B, k, k, n),
            200,
            || {
                gemm_ref(&mut c_ref, &a, &b, B, k, n);
            },
        );
        let t_tiled = bench_fn(
            &format!("sgemm_m64_tiled [{}×{}]@[{}×{}]", B, k, k, n),
            200,
            || unsafe {
                sgemm_m64_tiled(c_ours.as_mut_ptr(), a.as_ptr(), b.as_ptr(), B, k, n);
            },
        );
    ...
    for &(r, c) in &[(512usize, 1024usize), (1024, 1024), (1024, 512), (512, 256)] {
        println!("SGD [{}×{}]:", r, c);
        let mut act = vec![0f32; B * r];
        lcg(&mut act, &mut rng);
        let mut delta = vec![0f32; B * c];
        lcg(&mut delta, &mut rng);
        let mut w1 = vec![0.1f32; r * c];
        let mut w2 = vec![0.1f32; r * c];

        let t_ref = bench_fn(&format!("gemm_crate SGD [{}×{}]", r, c), 200, || unsafe {
            gemm::gemm(
                r,
                c,
                B,
        ...
        let t_ours = bench_fn(&format!("sgd_m64_tiled   [{}×{}]", r, c), 200, || unsafe {
            sgd_m64_tiled(w2.as_mut_ptr(), act.as_ptr(), delta.as_ptr(), r, B, c, lr);
        });
        println!("  speedup={:.2}x\n", t_ref / t_ours);
    }

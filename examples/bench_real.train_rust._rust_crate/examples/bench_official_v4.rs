...
fn fpar() -> faer::Par { faer::get_global_parallelism() }

/// C[m×n] = A[m×k] @ B[k×n]   (replace)
#[inline(always)]
fn fgemm(c: *mut f32, a: *const f32, b: *const f32, m: usize, k: usize, n: usize) {
    unsafe {
        matmul(as_mat_mut(c,m,n), Accum::Replace,
               as_mat(a,m,k), as_mat(b,k,n), 1.0f32, fpar());
    }
}
...
fn fsgd(w: *mut f32, a: *const f32, b: *const f32, r: usize, batch: usize, c: usize, lr: f32) {
    unsafe {
        matmul(as_mat_mut(w,r,c), Accum::Add,
               as_mat(a,batch,r).transpose(), as_mat(b,batch,c), -lr, fpar());
    }
}
...
fn bwd_wt(dp: *mut f32, w: *const f32, d: *const f32, r: usize, c: usize, batch: usize) {
    ...
    unsafe { gemm::gemm(batch,r,c, dp,1,r as isize,false, d,1,c as isize,
        w,c as isize,1, 0f32,1f32,false,false,false, par(batch,c,r)); }
}
...
fn bwd_wt_faer(dp: *mut f32, w: *const f32, d: *const f32, r: usize, c: usize, batch: usize) {
    unsafe {
        matmul(as_mat_mut(dp,batch,r), Accum::Replace,
               as_mat(d,batch,c), as_mat(w,r,c).transpose(), 1.0f32, fpar());
    }
}
...
unsafe fn bias_relu_avx512(pre: *mut f32, act: *mut f32, bias: *const f32, rows: usize, cols: usize) {
    ...
    for bi in 0..rows {
        let base = bi*cols; let mut j=0usize;
        while j+16<=cols {
        ...
        if j<cols {
            let rem=(cols-j) as u16; let mask=(1u16<<rem)-1;
...
unsafe fn relu_mask_db_avx512(delta: *mut f32, pre: *const f32, db: *mut f32, rows: usize, cols: usize) {
    ...
    for bi in 0..rows {
        ...
        if j<cols {
            let rem=(cols-j) as u16; let km=(1u16<<rem)-1;
...
unsafe fn transpose_8x8(dst: *mut f32, drows: usize, src: *const f32, scols: usize, bi: usize, bj: usize) {
    let r0=_mm256_loadu_ps(src.add(bi*scols+bj));     let r1=_mm256_loadu_ps(src.add((bi+1)*scols+bj));
    let r2=_mm256_loadu_ps(src.add((bi+2)*scols+bj)); let r3=_mm256_loadu_ps(src.add((bi+3)*scols+bj));
    let r4=_mm256_loadu_ps(src.add((bi+4)*scols+bj)); let r5=_mm256_loadu_ps(src.add((bi+5)*scols+bj));
    let r6=_mm256_loadu_ps(src.add((bi+6)*scols+bj)); let r7=_mm256_loadu_ps(src.add((bi+7)*scols+bj));
    ...
    _mm256_storeu_ps(dst.add(bj*drows+bi),o0);     _mm256_storeu_ps(dst.add((bj+1)*drows+bi),o1);
    _mm256_storeu_ps(dst.add((bj+2)*drows+bi),o2); _mm256_storeu_ps(dst.add((bj+3)*drows+bi),o3);
    _mm256_storeu_ps(dst.add((bj+4)*drows+bi),o4); _mm256_storeu_ps(dst.add((bj+5)*drows+bi),o5);
    _mm256_storeu_ps(dst.add((bj+6)*drows+bi),o6); _mm256_storeu_ps(dst.add((bj+7)*drows+bi),o7);
}

fn fast_transpose(dst: *mut f32, src: *const f32, rows: usize, cols: usize) {
    #[cfg(target_arch = "x86_64")]
    if rows%8==0 && cols%8==0 {
        let mut i=0; while i<rows { let mut j=0; while j<cols {
            unsafe { transpose_8x8(dst,rows,src,cols,i,j); } j+=8; } i+=8; }
        return;
    }
    const T:usize=32;
    let mut i=0; while i<rows { let im=(i+T).min(rows); let mut j=0; while j<cols {
        let jm=(j+T).min(cols);
        unsafe{for ii in i..im{for jj in j..jm{*dst.add(jj*rows+ii)=*src.add(ii*cols+jj);}}}
        j+=T;} i+=T;}
}
...
const N:usize=5; const B:usize=64;

struct State {
    ws: [Vec<f32>;N], bs: [Vec<f32>;N], dbs:[Vec<f32>;N],
    dts:[Vec<f32>;N], tmps:[Vec<f32>;N],
    acts:[Vec<f32>;6], pres:[Vec<f32>;4], deltas:[Vec<f32>;N],
    last_loss:f32,
}
impl State {
    fn new() -> Self {
        ...
        lcg_xavier(&mut ws[0],512,1024,&mut rng); lcg_xavier(&mut ws[1],1024,1024,&mut rng);
        lcg_xavier(&mut ws[2],1024,512,&mut rng);  lcg_xavier(&mut ws[3],512,256,&mut rng);
        lcg_xavier(&mut ws[4],256,1,&mut rng);
        State {
            ws, bs:std::array::from_fn(|i|vec![0f32;LAYERS[i+1]]),
    ...
    fn step(&mut self, x:&[f32], y:&[f32], lr:f32) {
        ...
        for i in 0..N {
            ...
            let is_last=i==N-1;
            if is_last {
                fgemm(self.acts[N].as_mut_ptr(), self.acts[i].as_ptr(), self.ws[i].as_ptr(), B,r,c);
                for bi in 0..B { for j in 0..c { self.acts[N][bi*c+j]+=self.bs[i][j]; } }
            } else {
                fgemm(self.pres[i].as_mut_ptr(), self.acts[i].as_ptr(), self.ws[i].as_ptr(), B,r,c);
                #[cfg(target_arch="x86_64")]
                unsafe { bias_relu_avx512(self.pres[i].as_mut_ptr(),self.acts[i+1].as_mut_ptr(),self.bs[i].as_ptr(),B,c); }
                #[cfg(not(target_arch="x86_64"))]
                for bi in 0..B { for j in 0..c {
                    let v=self.pres[i][bi*c+j]+self.bs[i][j];
                    self.pres[i][bi*c+j]=v; self.acts[i+1][bi*c+j]=if v>0.0{v}else{0.0};
                }}
            }
        }

        // ── LOSS ──────────────────────────────────────────────────────────
        let nt=B*LAYERS[N]; let mut lacc=0f32;
        for k in 0..nt { let d=self.acts[N][k]-y[k]; lacc+=d*d; self.deltas[N-1][k]=2.0*d/(nt as f32); }
        ...
        for i in (0..N).rev() {
            ...
            if !is_last {
                #[cfg(target_arch="x86_64")]
                unsafe { relu_mask_db_avx512(self.deltas[i].as_mut_ptr(),self.pres[i].as_ptr(),self.dbs[i].as_mut_ptr(),B,c); }
                #[cfg(not(target_arch="x86_64"))]
                { self.dbs[i].fill(0.0);
                  for bi in 0..B { for j in 0..c {
                    let m=if self.pres[i][bi*c+j]>0.0{1f32}else{0f32};
                    let d=self.deltas[i][bi*c+j]*m; self.deltas[i][bi*c+j]=d; self.dbs[i][j]+=d;
                  }}
                }
            ...
            if i > 0 {
                if r > c {
                    ...
                    bwd_wt(self.deltas[i-1].as_mut_ptr(), self.ws[i].as_ptr(), self.deltas[i].as_ptr(), r,c,B);
                } else {
                    // square/expanding: faer with explicit W^T is fastest
                    bwd_wt_faer(self.deltas[i-1].as_mut_ptr(), self.ws[i].as_ptr(), self.deltas[i].as_ptr(), r,c,B);
                }
            }

            // SGD (faer)
            fsgd(self.ws[i].as_mut_ptr(), self.acts[i].as_ptr(), self.deltas[i].as_ptr(), r,B,c,lr);
...
fn main() {
    ...
    const STEPS:usize=50; const RUNS:usize=7;
    ...
    println!("[TRAIN][VOLTA-RUST-V4] median={:.3} ms/step  all7={:?}",
        results[RUNS/2], results.map(|x|format!("{:.3}",x)));
    println!("  checksum={:.6}", checksum);
    println!("  batch={}  steps={}  in={}  out={}", B, STEPS, LAYERS[0], LAYERS[N]);
    #[cfg(windows)]
    unsafe { extern "system" { fn ExitProcess(code:u32); } ExitProcess(0); }
}

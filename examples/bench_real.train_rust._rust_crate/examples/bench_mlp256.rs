// bench_mlp256.rs — MLP 256→512→512→256→1, B=64, same protocol
// Build: cargo build --release --example bench_mlp256
...
fn sgd_fused_tn(W: *mut f32, A: *const f32, B_ptr: *const f32, m: usize, k: usize, n: usize, lr: f32) {
    unsafe {
        cblas_sgemm(101, 112, 111, m as i32, n as i32, k as i32,
            -lr, A, m as i32, B_ptr, n as i32, 1.0f32, W, n as i32);
    }
}
...
// MLP: 256→512→512→256→1, B=64
const LAYERS: [usize; 6] = [256, 512, 512, 256, 1, 0]; // last 0 unused
const NL: usize = 4; // 4 actual layers (5 sizes, 4 weight matrices)
// Sizes: [256,512,512,256,1]
const L: [usize; 5] = [256, 512, 512, 256, 1];
const B: usize = 64;

struct State {
    ws:  [Vec<f32>; NL], bs:  [Vec<f32>; NL], dbs: [Vec<f32>; NL],
    tmps: [Vec<f32>; NL], dts: [Vec<f32>; NL],
    acts: [Vec<f32>; 5], pres: [Vec<f32>; 3],
    deltas: [Vec<f32>; NL],
    last_loss: f32,
}
...
impl State {
    fn new() -> Self {
        ...
        for i in 0..NL { lcg_xavier(&mut ws[i], L[i], L[i+1], &mut rng); }
        State {
            ws,
    ...
    fn step(&mut self, x: &[f32], y: &[f32], lr: f32) {
        ...
        for i in 0..NL {
            ...
            let is_last = i == NL-1;
            if is_last {
                sgemm(self.acts[NL].as_mut_ptr(), self.acts[i].as_ptr(), self.ws[i].as_ptr(), B, r, c);
                for bi in 0..B { for j in 0..c { self.acts[NL][bi*c+j] += self.bs[i][j]; } }
            } else {
                sgemm(self.pres[i].as_mut_ptr(), self.acts[i].as_ptr(), self.ws[i].as_ptr(), B, r, c);
...
fn main() {
    ...
    let out_file = args.windows(2).find(|w| w[0] == "--out").map(|w| w[1].clone());

    let x: Vec<f32> = (0..B*L[0]).map(|i| (i%17) as f32 * 0.01 - 0.08).collect();
    ...
    eprintln!("[TRAIN][VOLTA-MLP256] median={:.3} ms/step  all7={:?}",
        med, results.map(|x| format!("{:.3}", x)));
    eprintln!("  checksum={:.6}  B={}  arch=256->512->512->256->1", checksum, B);

...
fn fgemm(c: *mut f32, a: *const f32, b: *const f32, m: usize, k: usize, n: usize) {
    unsafe {
        matmul(as_mat_mut(c,m,n), Accum::Replace, as_mat(a,m,k), as_mat(b,k,n), 1.0f32, fpar());
    }
}
...
fn fsgd(w: *mut f32, a: *const f32, d: *const f32, r: usize, batch: usize, c: usize, lr: f32) {
    unsafe {
        matmul(as_mat_mut(w,r,c), Accum::Add,
               as_mat(a,batch,r).transpose(), as_mat(d,batch,c), -lr, fpar());
    }
}
...
fn bwd_wt_stride(dp: *mut f32, w: *const f32, d: *const f32, r: usize, c: usize, batch: usize) {
    unsafe { gemm::gemm(batch,r,c, dp,1,r as isize,false, d,1,c as isize,
        w,c as isize,1isize, 0f32,1f32,false,false,false, par_gemm(batch,c,r)); }
}
...
fn do_fwd(s: &mut Slot, ws: &[Vec<f32>;N], bs: &[Vec<f32>;N], x: &[f32]) {
    ...
    for i in 0..N {
        let (r,c) = (LA[i], LA[i+1]);
        if i == N-1 {
            fgemm(s.acts[N].as_mut_ptr(), s.acts[i].as_ptr(), ws[i].as_ptr(), B,r,c);
            for bi in 0..B { for j in 0..c { s.acts[N][bi*c+j] += bs[i][j]; } }
        } else {
            fgemm(s.pres[i].as_mut_ptr(), s.acts[i].as_ptr(), ws[i].as_ptr(), B,r,c);
            #[cfg(target_arch="x86_64")]
    ...
    for i in (0..N).rev() {
        ...
        if i > 0 {
            if r > c {
                ...
                bwd_wt_stride(s.deltas[i-1].as_mut_ptr(), ws[i].as_ptr(), s.deltas[i].as_ptr(), r,c,B);
            } else {
                // Growing/square layer: faer with explicit transpose
                bwd_wt_faer(s.deltas[i-1].as_mut_ptr(), ws[i].as_ptr(), s.deltas[i].as_ptr(), r,c,B);
            }
        }
        if i == N-1 {
            s.dbs[i].fill(0.0);
...
fn apply_sgd(s: &Slot, ws_out: &mut [Vec<f32>;N], lr: f32) {
    for i in 0..N {
        let (r,c) = (LA[i], LA[i+1]);
        fsgd(ws_out[i].as_mut_ptr(), s.acts[i].as_ptr(), s.deltas[i].as_ptr(), r,B,c,lr);
    }
}
...
fn main() {
    ...
    let mut rb = [0f64;7]; let mut rp = [0f64;7];
    println!("testing sequential...");
    for r in 0..7 { let (t,_) = run_sequential(&x,&y,lr,50); rb[r]=t; }
    println!("testing pipeline...");
    for r in 0..7 { let (t,_) = run_pipeline(&x,&y,lr,50); rp[r]=t; }
    rb.sort_by(|a,b|a.partial_cmp(b).unwrap());
    rp.sort_by(|a,b|a.partial_cmp(b).unwrap());
    println!("[sequential] median={:.3} ms  all={:?}", rb[3], rb.map(|v|format!("{:.3}",v)));
    println!("[pipeline  ] median={:.3} ms  all={:?}", rp[3], rp.map(|v|format!("{:.3}",v)));
    println!("speedup: {:.2}x  (PyTorch baseline ~2.440 ms)", rb[3]/rp[3]);
    println!("vs PyTorch: pipeline {:.1}% faster", (2.440-rp[3])/2.440*100.0);

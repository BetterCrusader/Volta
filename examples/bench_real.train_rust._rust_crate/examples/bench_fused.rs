...
fn sgemm_acc(c: *mut f32, a: *const f32, b: *const f32, m: usize, k: usize, n: usize) {
    // C += A @ B  (accumulate, alpha=1)
    unsafe { gemm::gemm(m,n,k, c,1isize,n as isize, true, a,1isize,k as isize, b,1isize,n as isize, 1f32,1f32, false,false,false, par(m,k,n)); }
}
...
fn bwd_delta_Wt(dp: *mut f32, w: *const f32, d: *const f32, r: usize, c: usize, batch: usize) {
    unsafe { gemm::gemm(batch,r,c, dp,1isize,r as isize, false, d,1isize,c as isize, w,c as isize,1isize, 0f32,1f32, false,false,false, par(batch,c,r)); }
}

// ── Baseline: separate fwd / bwd / sgd calls ─────────────────────────────────
fn baseline_step(
    w: &mut [f32], act_in: &[f32], act_out: &mut [f32],
    pre: &mut [f32], delta_in: &[f32], delta_prev: &mut [f32],
    r: usize, c: usize, batch: usize, lr: f32,
) {
    // fwd: act_out[B×c] = act_in[B×r] @ W[r×c]
    sgemm(act_out.as_mut_ptr(), act_in.as_ptr(), w.as_ptr(), batch, r, c);
    // bwd: delta_prev[B×r] = delta_in[B×c] @ W^T
    if r > c {
        bwd_delta_Wt(delta_prev.as_mut_ptr(), w.as_ptr(), delta_in.as_ptr(), r, c, batch);
    } else {
        // transpose + sgemm + transpose
        let mut dt = vec![0f32; c*batch];
        let mut tmp = vec![0f32; r*batch];
        const T: usize=32;
        let mut i=0; while i<batch { let im=(i+T).min(batch); let mut j=0; while j<c {
            let jm=(j+T).min(c);
            unsafe{for ii in i..im{for jj in j..jm{*dt.as_mut_ptr().add(jj*batch+ii)=*delta_in.as_ptr().add(ii*c+jj);}}}
            j+=T;} i+=T;}
        sgemm(tmp.as_mut_ptr(), w.as_ptr(), dt.as_ptr(), r, c, batch);
        let mut i=0; while i<r { let im=(i+T).min(r); let mut j=0; while j<batch {
            let jm=(j+T).min(batch);
            unsafe{for ii in i..im{for jj in j..jm{*delta_prev.as_mut_ptr().add(jj*r+ii)=*tmp.as_ptr().add(ii*batch+jj);}}}
            j+=T;} i+=T;}
    }
    // sgd: W -= lr * act_in^T @ delta_in
    sgd_fused_tn(w.as_mut_ptr(), act_in.as_ptr(), delta_in.as_ptr(), r, batch, c, lr);
    let _ = pre;
}
...
fn fused_step(
    w: &mut [f32], act_in: &[f32], act_out: &mut [f32],
    delta_in: &[f32], delta_prev: &mut [f32],
    r: usize, c: usize, batch: usize, lr: f32,
) {
    // zero outputs first
    ...
    while ri < r {
        ...
        let ap_tile = unsafe { act_out.as_mut_ptr() };         // act_out[batch, 0..c]
        let dp_tile = unsafe { delta_prev.as_mut_ptr().add(ri) }; // delta_prev[batch, ri..ri+rt]
        let d_tile  = unsafe { delta_in.as_ptr() };            // delta_in[batch, 0..c]
        ...
        unsafe { gemm::gemm(
            batch, c, rt,
            ap_tile, 1isize, c as isize, ri > 0, // accumulate after first tile
            a_tile, r as isize, 1isize,           // act_in col-major: col-stride=r, row-stride=1
            w_tile, 1isize, c as isize,
            if ri > 0 { 1f32 } else { 0f32 }, 1f32,
            false, false, false,
            par(batch, rt, c),
        ); }
        ...
        unsafe { gemm::gemm(
            batch, rt, c,
            dp_tile, r as isize, 1isize, ri > 0, // col-major write, accumulate
            d_tile, 1isize, c as isize,
            w_tile, c as isize, 1isize,           // W as W^T
            if ri > 0 { 1f32 } else { 0f32 }, 1f32,
            false, false, false,
            par(batch, c, rt),
        ); }
        ...
        unsafe { gemm::gemm(
            rt, c, batch,
            w_tile, 1isize, c as isize, true,
            a_tile, r as isize, 1isize,  // act_in^T: reading act_in col-major
            d_tile, 1isize, c as isize,
            1f32, -lr,
            false, false, false,
            par(rt, batch, c),
...
fn bench(label: &str, mut f: impl FnMut()) -> f64 {
    ...
    let med = results[3];
    println!("[{}] median={:.1} us  all={:?}", label, med, results.map(|x| format!("{:.0}",x)));
    med
}
...
fn main() {
    ...
    for &(r, c) in &[(512usize, 1024usize), (1024, 1024), (1024, 512), (512, 256)] {
        let mut w_base = vec![0f32; r*c]; lcg_fill(&mut w_base, &mut rng);
        let mut w_fuse = w_base.clone();
        let act_in = {let mut v=vec![0f32;batch*r]; lcg_fill(&mut v,&mut rng); v};
        let delta_in = {let mut v=vec![0f32;batch*c]; lcg_fill(&mut v,&mut rng); v};
        ...
        baseline_step(&mut w_base, &act_in, &mut ao_base, &mut pre, &delta_in, &mut dp_base, r, c, batch, lr);
        fused_step(&mut w_fuse, &act_in, &mut ao_fuse, &delta_in, &mut dp_fuse, r, c, batch, lr);
    ...
    for &(r, c) in &[(512usize, 1024usize), (1024, 1024), (1024, 512), (512, 256)] {
        let mut rng2 = 99u64;
        let mut w = vec![0f32; r*c]; lcg_fill(&mut w, &mut rng2);
        let act_in = {let mut v=vec![0f32;batch*r]; lcg_fill(&mut v,&mut rng2); v};
        let delta_in = {let mut v=vec![0f32;batch*c]; lcg_fill(&mut v,&mut rng2); v};
        let mut pre = vec![0f32; batch*c];
        let mut ao = vec![0f32; batch*c];
        let mut dp = vec![0f32; batch*r];

        let mut wb = w.clone(); let mut wf = w.clone();

        println!("--- {}x{} ---", r, c);
        bench(&format!("baseline {}x{}", r, c), || {
            baseline_step(&mut wb, &act_in, &mut ao, &mut pre, &delta_in, &mut dp, r, c, batch, lr);
        });
        bench(&format!("fused    {}x{}", r, c), || {
            fused_step(&mut wf, &act_in, &mut ao, &delta_in, &mut dp, r, c, batch, lr);
        });
    }

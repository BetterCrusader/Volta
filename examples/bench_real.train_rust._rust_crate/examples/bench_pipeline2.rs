// bench_pipeline2.rs — 3-way parallel pipeline (best version):
//   Thread A: fwd(N+1)
...
fn par(m: usize, k: usize, n: usize) -> gemm::Parallelism {
    let t = PAR_THREADS.load(std::sync::atomic::Ordering::Relaxed);
    let ops = 2*m*k*n;
    if ops < (1<<20) { gemm::Parallelism::None }
    else if ops < (1<<25) { gemm::Parallelism::Rayon(t) }
...
fn compute_grads(s: &mut Slot, ws: &[Vec<f32>;N], bs: &mut [Vec<f32>;N], y: &[f32], lr: f32) {
    ...
    for i in (0..N).rev() {
        ...
        if i != N-1 {
            #[cfg(target_arch="x86_64")]
            unsafe { relu_mask_db(s.deltas[i].as_mut_ptr(), s.pres[i].as_ptr(), s.dbs[i].as_mut_ptr(), B, c); }
...
fn apply_sgd(s: &Slot, ws_out: &mut [Vec<f32>;N], lr: f32) {
    for i in 0..N {
        let (r,c) = (LA[i], LA[i+1]);
        sgd_tn(ws_out[i].as_mut_ptr(), s.acts[i].as_ptr(), s.deltas[i].as_ptr(), r,B,c,lr);
    }
}
...
fn copy_ws(dst: &mut [Vec<f32>;N], src: &[Vec<f32>;N]) {
    for i in 0..N { dst[i].copy_from_slice(&src[i]); }
}
fn copy_bs(dst: &mut [Vec<f32>;N], src: &[Vec<f32>;N]) {
    for i in 0..N { dst[i].copy_from_slice(&src[i]); }
}

fn run_sequential(x: &[f32], y: &[f32], lr: f32, steps: usize) -> (f64, f32) {
    let mut ws: [Vec<f32>;N] = make_ws();
...
fn run_pipeline(x: &[f32], y: &[f32], lr: f32, steps: usize) -> (f64, f32) {
    ...
    for _step in 0..steps {
        ...
        rayon::join(
            ...
            || unsafe {
                // Thread B: copy_ws || compute_grads  (nested parallel)
...
fn main() {
    ...
    // Tune parallelism: try different Rayon thread counts
    println!("=== Tuning par_threads for pipeline ===");
    for t in [1usize, 2, 3, 4, 5, 6, 8, 0] {
        PAR_THREADS.store(t, std::sync::atomic::Ordering::Relaxed);
        let mut rp = [0f64;7];
        for r in 0..7 { let (v,_) = run_pipeline(&x,&y,lr,50); rp[r]=v; }
        rp.sort_by(|a,b|a.partial_cmp(b).unwrap());
        let label = if t == 0 { "all".to_string() } else { t.to_string() };
        println!("  par_threads={:<3} → pipeline p50={:.3} ms  p25={:.3}", label, rp[3], rp[1]);
    }

    // Best sequential baseline
    PAR_THREADS.store(0, std::sync::atomic::Ordering::Relaxed);
    let mut rb = [0f64;7];
    for r in 0..7 { let (t,_) = run_sequential(&x,&y,lr,50); rb[r]=t; }
    ...
    PAR_THREADS.store(0, std::sync::atomic::Ordering::Relaxed);
    let mut rb2 = [0f64;7];
    for r in 0..7 { let (t,_) = run_sequential(&x,&y,lr,50); rb2[r]=t; }
    rb2.sort_by(|a,b|a.partial_cmp(b).unwrap());

    for best_t in [5usize] {
        PAR_THREADS.store(best_t, std::sync::atomic::Ordering::Relaxed);
        let mut rp2 = [0f64;7];
        for r in 0..7 { let (t,_) = run_pipeline(&x,&y,lr,50); rp2[r]=t; }
        rp2.sort_by(|a,b|a.partial_cmp(b).unwrap());
        println!("[sequential] median={:.3} ms  all={:?}", rb2[3], rb2.map(|v|format!("{:.3}",v)));
        println!("[pipeline t={}] median={:.3} ms  all={:?}", best_t, rp2[3], rp2.map(|v|format!("{:.3}",v)));
        println!("speedup: {:.2}x", rb2[3]/rp2[3]);
    }

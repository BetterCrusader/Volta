...
unsafe fn transpose_16x16_avx512(dst: *mut f32, dst_rows: usize, src: *const f32, src_cols: usize, bi: usize, bj: usize) {
    use std::arch::x86_64::*;
    ...
    // unpack lo/hi pairs
    let t0  = _mm512_unpacklo_ps(r0,  r1 ); let t1  = _mm512_unpackhi_ps(r0,  r1 );
    let t2  = _mm512_unpacklo_ps(r2,  r3 ); let t3  = _mm512_unpackhi_ps(r2,  r3 );
    let t4  = _mm512_unpacklo_ps(r4,  r5 ); let t5  = _mm512_unpackhi_ps(r4,  r5 );
    let t6  = _mm512_unpacklo_ps(r6,  r7 ); let t7  = _mm512_unpackhi_ps(r6,  r7 );
    let t8  = _mm512_unpacklo_ps(r8,  r9 ); let t9  = _mm512_unpackhi_ps(r8,  r9 );
    let t10 = _mm512_unpacklo_ps(r10, r11); let t11 = _mm512_unpackhi_ps(r10, r11);
    let t12 = _mm512_unpacklo_ps(r12, r13); let t13 = _mm512_unpackhi_ps(r12, r13);
    let t14 = _mm512_unpacklo_ps(r14, r15); let t15 = _mm512_unpackhi_ps(r14, r15);
    // shuffle into 4-wide groups
    let s0  = _mm512_shuffle_ps(t0,  t2,  0x44); let s1  = _mm512_shuffle_ps(t0,  t2,  0xEE);
    let s2  = _mm512_shuffle_ps(t1,  t3,  0x44); let s3  = _mm512_shuffle_ps(t1,  t3,  0xEE);
    let s4  = _mm512_shuffle_ps(t4,  t6,  0x44); let s5  = _mm512_shuffle_ps(t4,  t6,  0xEE);
    let s6  = _mm512_shuffle_ps(t5,  t7,  0x44); let s7  = _mm512_shuffle_ps(t5,  t7,  0xEE);
    let s8  = _mm512_shuffle_ps(t8,  t10, 0x44); let s9  = _mm512_shuffle_ps(t8,  t10, 0xEE);
    let s10 = _mm512_shuffle_ps(t9,  t11, 0x44); let s11 = _mm512_shuffle_ps(t9,  t11, 0xEE);
    let s12 = _mm512_shuffle_ps(t12, t14, 0x44); let s13 = _mm512_shuffle_ps(t12, t14, 0xEE);
    let s14 = _mm512_shuffle_ps(t13, t15, 0x44); let s15 = _mm512_shuffle_ps(t13, t15, 0xEE);
    // permute 128-bit lanes to get 8-wide groups
    let u0  = _mm512_shuffle_f32x4(s0,  s4,  0x44); let u1  = _mm512_shuffle_f32x4(s1,  s5,  0x44);
    let u2  = _mm512_shuffle_f32x4(s2,  s6,  0x44); let u3  = _mm512_shuffle_f32x4(s3,  s7,  0x44);
    let u4  = _mm512_shuffle_f32x4(s0,  s4,  0xEE); let u5  = _mm512_shuffle_f32x4(s1,  s5,  0xEE);
    let u6  = _mm512_shuffle_f32x4(s2,  s6,  0xEE); let u7  = _mm512_shuffle_f32x4(s3,  s7,  0xEE);
    let u8  = _mm512_shuffle_f32x4(s8,  s12, 0x44); let u9  = _mm512_shuffle_f32x4(s9,  s13, 0x44);
    let u10 = _mm512_shuffle_f32x4(s10, s14, 0x44); let u11 = _mm512_shuffle_f32x4(s11, s15, 0x44);
    let u12 = _mm512_shuffle_f32x4(s8,  s12, 0xEE); let u13 = _mm512_shuffle_f32x4(s9,  s13, 0xEE);
    let u14 = _mm512_shuffle_f32x4(s10, s14, 0xEE); let u15 = _mm512_shuffle_f32x4(s11, s15, 0xEE);
    // final 256-bit lane permute to get full 16-wide rows
    let o0  = _mm512_shuffle_f32x4(u0,  u8,  0x44); let o1  = _mm512_shuffle_f32x4(u1,  u9,  0x44);
    let o2  = _mm512_shuffle_f32x4(u2,  u10, 0x44); let o3  = _mm512_shuffle_f32x4(u3,  u11, 0x44);
    let o4  = _mm512_shuffle_f32x4(u4,  u12, 0x44); let o5  = _mm512_shuffle_f32x4(u5,  u13, 0x44);
    let o6  = _mm512_shuffle_f32x4(u6,  u14, 0x44); let o7  = _mm512_shuffle_f32x4(u7,  u15, 0x44);
    let o8  = _mm512_shuffle_f32x4(u0,  u8,  0xEE); let o9  = _mm512_shuffle_f32x4(u1,  u9,  0xEE);
    let o10 = _mm512_shuffle_f32x4(u2,  u10, 0xEE); let o11 = _mm512_shuffle_f32x4(u3,  u11, 0xEE);
    let o12 = _mm512_shuffle_f32x4(u4,  u12, 0xEE); let o13 = _mm512_shuffle_f32x4(u5,  u13, 0xEE);
    let o14 = _mm512_shuffle_f32x4(u6,  u14, 0xEE); let o15 = _mm512_shuffle_f32x4(u7,  u15, 0xEE);
...
fn fast_transpose(dst: *mut f32, src: *const f32, rows: usize, cols: usize) {
    const T: usize = 32;
    let mut i=0; while i<rows { let imax=(i+T).min(rows); let mut j=0; while j<cols {
...
fn prefetch_ro(ptr: *const f32, bytes: usize) {
    ...
    while p < end {
        unsafe { _mm_prefetch(p, _MM_HINT_T1); }
        p = unsafe { p.add(64) };
    }
}
...
unsafe fn fused_bwd_sgd_avx2(
    ...
    r: usize, c: usize, batch: usize, lr: f32,
) {
    use std::arch::x86_64::*;
    ...
    while ri + R_TILE <= r {
        ...
        for bi in 0..batch {
            ...
            for t in 0usize..R_TILE {
                ...
                // scalar tail
                while j < c { acc += *d_row.add(j) * *w_row.add(j); j += 1; }
        ...
        for t in 0usize..R_TILE {
            ...
            while j < c {
                let mut grad = 0.0f32;
                for bi in 0..batch { grad += *acts.add(bi * r + ri + t) * *delta.add(bi * c + j); }
                *w_row.add(j) -= lr * grad;
                j += 1;
            }
        }
    ...
    while ri < r {
        ...
        while j < c {
            let mut grad = 0.0f32;
            for bi in 0..batch { grad += *acts.add(bi * r + ri) * *delta.add(bi * c + j); }
            *w_row.add(j) -= lr * grad;
            j += 1;
        }
        ri += 1;
...
#[cfg(target_arch = "x86_64")]
#[target_feature(enable = "avx512f")]
unsafe fn bias_relu_avx512(pre: *mut f32, act: *mut f32, bias: *const f32, rows: usize, cols: usize) {
    use std::arch::x86_64::*;
...
#[cfg(target_arch = "x86_64")]
#[target_feature(enable = "avx512f")]
unsafe fn relu_mask_db_avx512(delta: *mut f32, pre: *const f32, db: *mut f32, rows: usize, cols: usize) {
    use std::arch::x86_64::*;
...
impl State {
    ...
    fn step(&mut self, x: &[f32], y: &[f32], lr: f32) {
        ...
        for i in 0..N {
            ...
            sgemm(if is_last { self.acts[N].as_mut_ptr() } else { self.pres[i].as_mut_ptr() },
                  self.acts[i].as_ptr(), self.ws[i].as_ptr(), B, r, c);

            if is_last {
                for bi in 0..B { for j in 0..c { self.acts[N][bi*c+j] += self.bs[i][j]; } }
            } else {
                #[cfg(target_arch = "x86_64")]
                unsafe {
                    bias_relu_avx512(
        ...
        for i in (0..N).rev() {
            ...
            if !is_last {
                ...
                #[cfg(not(target_arch = "x86_64"))]
                {
                    self.dbs[i].fill(0.0);
                    for bi in 0..B { for j in 0..c {
                        let mask = if self.pres[i][bi*c+j] > 0.0 { 1.0f32 } else { 0.0 };
                        let d = self.deltas[i][bi*c+j] * mask;
                        self.deltas[i][bi*c+j] = d;
                        self.dbs[i][j] += d;
                    }}
                }
                for k in 0..c { self.bs[i][k] -= lr * self.dbs[i][k]; }
            }
            ...
            if i > 0 {
                ...
                if r > c {
                    bwd_delta_Wt(self.deltas[i-1].as_mut_ptr(), self.ws[i].as_ptr(), self.deltas[i].as_ptr(), r, c, B);
                } else {
                    fast_transpose(self.dts[i].as_mut_ptr(), self.deltas[i].as_ptr(), B, c);
                    sgemm(self.tmps[i].as_mut_ptr(), self.ws[i].as_ptr(), self.dts[i].as_ptr(), r, c, B);
                    fast_transpose(self.deltas[i-1].as_mut_ptr(), self.tmps[i].as_ptr(), r, B);
                }
            }

            sgd_fused_tn(self.ws[i].as_mut_ptr(), self.acts[i].as_ptr(), self.deltas[i].as_ptr(), r, B, c, lr);

            if is_last {
                self.dbs[i].fill(0.0);
                for bi in 0..B { for j in 0..c { self.dbs[i][j] += self.deltas[i][bi*c+j]; } }
                for k in 0..c { self.bs[i][k] -= lr * self.dbs[i][k]; }
            }
        }
    }
...
fn main() {
    let x: Vec<f32> = (0..B*LAYERS[0]).map(|i| (i%17) as f32 * 0.01 - 0.08).collect();
    let y: Vec<f32> = (0..B*LAYERS[N]).map(|i| (i%7) as f32 * 0.1).collect();
    let lr = 0.01f32;

    let mut s = State::new();
    for _ in 0..10 { s.step(&x, &y, lr); }
    ...
    for r in 0..RUNS {
        let t0 = Instant::now();
        for _ in 0..STEPS { s.step(&x, &y, lr); }
        results[r] = t0.elapsed().as_nanos() as f64 / 1000.0 / STEPS as f64 / 1000.0;
        checksum += s.last_loss;
    }
    results.sort_by(|a, b| a.partial_cmp(b).unwrap());
    let med = results[RUNS/2];
    println!("[TRAIN][VOLTA-RUST-V3] median={:.3} ms/step  all7={:?}",
        med, results.map(|x| format!("{:.3}", x)));
    println!("  checksum={:.6}", checksum);
    println!("  batch={}  steps={}  in={}  out={}", B, STEPS, LAYERS[0], LAYERS[N]);

    #[cfg(windows)]
    unsafe {
        extern "system" { fn ExitProcess(code: u32); }
        ExitProcess(0);
    }
}

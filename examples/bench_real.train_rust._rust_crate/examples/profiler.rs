...
impl Timings {
    fn total(&self) -> f64 {
        self.gemm_fwd + self.bias_relu + self.loss + self.relu_mask
            + self.transpose + self.gemm_bwd + self.sgd
    }
...
impl State {
    ...
    fn step_timed(&mut self, x: &[f32], y: &[f32], lr: f32, t: &mut Timings) {
        ...
        for i in 0..N {
            ...
            if is_last {
                let t0 = Instant::now();
                sgemm(self.acts[N].as_mut_ptr(), self.acts[i].as_ptr(), self.ws[i].as_ptr(), B, r, c);
...
fn print_row(label: &str, us: f64, pct: f64) {
    println!("  {:<14} {:>7.1} us  {:>5.1}%  [{}]", label, us, pct, bar(pct, 40));
}
...
fn main() {
    ...
    for _ in 0..WARMUP { s.step_timed(&x, &y, lr, &mut dummy); }
    ...
    println!();
    println!("=================================================================");
    println!("  VOLTA PROFILER  —  MLP 512->1024->1024->512->256->1  B={}  ", B);
    println!("  {} steps  (warmup={})  per-step averages", STEPS, WARMUP);
    println!("=================================================================");
    println!("  {:<14} {:>7}     {:>5}   {}", "section", "time", "share", "bar (40 chars = 100%)");
    println!("  {}", "-".repeat(72));
    ...
    println!("  {}", "-".repeat(72));
    println!("  {:<14} {:>7.1} us  100.0%  (wall per step)", "TOTAL", wall_per_step);
    println!("=================================================================");
    println!();
    println!("  gemm total (fwd+bwd+sgd): {:.1} us  ({:.1}%)",
        acc.gemm_fwd + acc.gemm_bwd + acc.sgd,
        pct(acc.gemm_fwd + acc.gemm_bwd + acc.sgd));
    println!("  scalar loops (bias+relu+mask+loss): {:.1} us  ({:.1}%)",
        acc.bias_relu + acc.relu_mask + acc.loss,
        pct(acc.bias_relu + acc.relu_mask + acc.loss));
    println!("  memory (transpose): {:.1} us  ({:.1}%)",
        acc.transpose, pct(acc.transpose));
    println!();

    // checksum
    println!("  checksum (loss): {:.6}  batch={}  in={}  out={}",

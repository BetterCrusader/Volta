...
fn main() {
    ...
    for &(k, n) in &[(512usize,1024usize),(1024,1024),(1024,512),(512,256)] {
        println!("\n--- Forward: [{}x{}] @ [{}x{}] = [{}x{}] ---", B,k, k,n, B,n);
        let mut a = vec![0f32; B*k]; lcg_fill(&mut a, &mut rng);
        let mut b = vec![0f32; k*n]; lcg_fill(&mut b, &mut rng);
        let mut c = vec![0f32; B*n];

        // gemm crate
        bench_fn("gemm_crate C=A@B", || {
            unsafe { gemm::gemm(B,n,k, c.as_mut_ptr(),1,n as isize,false, a.as_ptr(),1,k as isize, b.as_ptr(),1,n as isize, 0f32,1f32,false,false,false, par_gemm(B,k,n)); }
        });
    ...
    for &(r, c) in &[(1024usize,512usize),(512,256),(256,1)] {
        println!("\n--- Backward (r>c): [{}x{}] @ [{}x{}]^T = [{}x{}] ---", B,c, r,c, B,r);
        let mut delta = vec![0f32; B*c]; lcg_fill(&mut delta, &mut rng);
        let mut w     = vec![0f32; r*c]; lcg_fill(&mut w, &mut rng);
        let mut dp    = vec![0f32; B*r];

        bench_fn("gemm_crate delta@W^T (stride)", || {
            unsafe { gemm::gemm(B,r,c, dp.as_mut_ptr(),1,r as isize,false, delta.as_ptr(),1,c as isize, w.as_ptr(),c as isize,1isize, 0f32,1f32,false,false,false, par_gemm(B,c,r)); }
        });
        ...
        bench_fn("faer delta@W^T (explicit transpose)", || {
            faer::linalg::matmul::matmul(
                fdp.as_mut(), faer::Accum::Replace,
                fd.as_ref(), fw.transpose(),
                1.0f32,
                faer::get_global_parallelism(),
            );
        });
    ...
    for &(r, c) in &[(512usize,1024usize),(1024,1024),(1024,512),(512,256)] {
        ...
        bench_fn(&format!("gemm_crate SGD {}x{}", r, c), || {
            unsafe { gemm::gemm(r,c,B, w1.as_mut_ptr(),1,c as isize,true, act.as_ptr(),r as isize,1isize, delta.as_ptr(),1,c as isize, 1f32,-lr,false,false,false, par_gemm(r,B,c)); }
        });
        ...
        bench_fn(&format!("faer SGD {}x{}", r, c), || {
            faer::linalg::matmul::matmul(
                fw2.as_mut(), faer::Accum::Add,
                fa2.transpose(), fd2.as_ref(),
                -lr,
                faer::get_global_parallelism(),
            );
        });

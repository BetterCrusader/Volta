...
fn bwd_transpose_sgemm(
    delta_prev: *mut f32, tmp: *mut f32, dt: *mut f32,
    w: *const f32, delta: *const f32,
    r: usize, c: usize, batch: usize,
) {
    // transpose delta [B×c] → dt [c×B]
    const T: usize = 32;
    let mut i=0; while i<batch { let imax=(i+T).min(batch); let mut j=0; while j<c {
        let jmax=(j+T).min(c);
        unsafe { for ii in i..imax { for jj in j..jmax { *dt.add(jj*batch+ii)=*delta.add(ii*c+jj); } } }
        j+=T; } i+=T; }
    // sgemm(tmp [r×B]) = W [r×c] @ dt [c×B]
    unsafe { gemm::gemm(r,batch,c, tmp,1isize,batch as isize, false, w,1isize,c as isize, dt,1isize,batch as isize, 0f32,1f32, false,false,false, par(r,c,batch)); }
    // transpose tmp [r×B] → delta_prev [B×r]
    let mut i=0; while i<r { let imax=(i+T).min(r); let mut j=0; while j<batch {
        let jmax=(j+T).min(batch);
        unsafe { for ii in i..imax { for jj in j..jmax { *delta_prev.add(jj*r+ii)=*tmp.add(ii*batch+jj); } } }
        j+=T; } i+=T; }
}
...
fn bwd_sgemm_trans_b(
    delta_prev: *mut f32, tmp: *mut f32,
    w: *const f32, delta: *const f32,
    r: usize, c: usize, batch: usize,
) {
    ...
    unsafe { gemm::gemm(
        r, batch, c,
        tmp, 1isize, batch as isize, false,
        w, 1isize, c as isize,
        delta, c as isize, 1isize,
        0f32, 1f32, false, false, false,
        par(r, c, batch)
    ); }
    // transpose tmp [r×B] → delta_prev [B×r]
...
fn bench_variant(label: &str, r: usize, c: usize, batch: usize, mut f: impl FnMut()) -> f64 {
    ...
    let med = results[3];
    println!("[{}] r={} c={} B={} median={:.1} us", label, r, c, batch, med);
    med
}
...
fn main() {
    ...
    for &(r, c) in &[(512usize, 1024usize), (1024, 1024), (1024, 512), (512, 256)] {
        let mut w = vec![0.0f32; r*c]; lcg_fill(&mut w, &mut rng);
        let mut delta = vec![0.0f32; batch*c]; lcg_fill(&mut delta, &mut rng);
        let mut delta_prev = vec![0.0f32; batch*r];
        let mut tmp = vec![0.0f32; r*batch];
        let mut dt = vec![0.0f32; c*batch];

        let (wp, dp, dpp, tmpp, dtp) = (w.as_ptr(), delta.as_ptr(), delta_prev.as_mut_ptr(), tmp.as_mut_ptr(), dt.as_mut_ptr());
        ...
        bench_variant("varB (sgemm_Wdt_no_dt_alloc + 1 transpose)", r, c, batch, || {

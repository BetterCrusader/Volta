...
unsafe fn bwd_copy_sgd(
    ...
    // Layer N-1 (output layer): no bwd_delta needed, just bias+SGD
    {
        ...
        for bi in 0..B {
            for j in 0..c {
                s.dbs[N - 1][j] += s.deltas[N - 1][bi * c + j];
            }
        }
        for k in 0..c {
            bd[N - 1][k] -= lr * s.dbs[N - 1][k];
        }
    }
...
fn run_pipeline(x: &[f32], y: &[f32], lr: f32, steps: usize) -> (f64, f32) {
    ...
    for _ in 0..10 {
        ...
        unsafe {
            bwd_copy_sgd(&mut slots[0], wsr, wsw, bsw, y.as_ptr(), lr);
        }
        ri = wi;
    }

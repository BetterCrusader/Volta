...
fn par(m: usize, k: usize, n: usize) -> gemm::Parallelism {
    ...
    else if ops < (1<<25) { gemm::Parallelism::Rayon(3) }
    else { gemm::Parallelism::Rayon(0) }
}
fn sgemm(c: *mut f32, a: *const f32, b: *const f32, m: usize, k: usize, n: usize) {
    unsafe { gemm::gemm(m,n,k,c,1,n as isize,false,a,1,k as isize,b,1,n as isize,0f32,1f32,false,false,false,par(m,k,n)); }
}
fn sgd_tn(w: *mut f32, a: *const f32, b: *const f32, m: usize, k: usize, n: usize, lr: f32) {
    unsafe { gemm::gemm(m,n,k,w,1,n as isize,true,a,m as isize,1,b,1,n as isize,1f32,-lr,false,false,false,par(m,k,n)); }
}
fn bwd_wt(dp: *mut f32, w: *const f32, d: *const f32, r: usize, c: usize, b: usize) {
    unsafe { gemm::gemm(b,r,c,dp,1,r as isize,false,d,1,c as isize,w,c as isize,1,0f32,1f32,false,false,false,par(b,c,r)); }
}
...
unsafe fn bias_relu(pre: *mut f32, act: *mut f32, bias: *const f32, rows: usize, cols: usize) {
    ...
    for bi in 0..rows {
        ...
        if j < cols {
            let rem = (cols-j) as u16; let mk = (1u16<<rem)-1;
...
unsafe fn relu_mask_db(delta: *mut f32, pre: *const f32, db: *mut f32, rows: usize, cols: usize) {
    ...
    for bi in 0..rows {
        ...
        if j < cols {
            ...
            _mm512_mask_storeu_ps(db.add(j), km,
...
fn do_fwd(s: &mut Slot, ws: &[Vec<f32>;N], bs: &[Vec<f32>;N], x: &[f32]) {
    ...
    for i in 0..N {
        let (r,c) = (LA[i], LA[i+1]);
        if i == N-1 {
            sgemm(s.acts[N].as_mut_ptr(), s.acts[i].as_ptr(), ws[i].as_ptr(), B,r,c);
            for bi in 0..B { for j in 0..c { s.acts[N][bi*c+j] += bs[i][j]; } }
        } else {
            sgemm(s.pres[i].as_mut_ptr(), s.acts[i].as_ptr(), ws[i].as_ptr(), B,r,c);
            #[cfg(target_arch="x86_64")]
            unsafe { bias_relu(s.pres[i].as_mut_ptr(), s.acts[i+1].as_mut_ptr(), bs[i].as_ptr(), B, c); }
        }
    }
}

// compute_grads + copy_ws + sgd all in one scope
// Trick: spawn SGD for each layer immediately after its delta is ready
// copy_ws happens sequentially before backward starts
unsafe fn bwd_copy_sgd(
    s: *mut Slot,
    ws_src: *const [Vec<f32>;N],
    ws_dst: *mut [Vec<f32>;N],
    bs_dst: *mut [Vec<f32>;N],
    y: *const f32, lr: f32,
) {
    let s = &mut *s;
    let ws = &*ws_src;
    ...
    // First: copy ws
    for i in 0..N { wd[i].copy_from_slice(&ws[i]); }

    // Backward pass with rayon::scope for async SGD spawning
    let nt = B;
    let yslice = std::slice::from_raw_parts(y, B*LA[N]);
    let mut lacc = 0f32;
    for k in 0..nt { let d = s.acts[N][k]-yslice[k]; lacc+=d*d; s.deltas[N-1][k]=2.0*d/(nt as f32); }
    s.loss = lacc/(nt as f32);

    // Layer N-1 (output layer): no bwd_delta needed, just bias+SGD
    {
        let c = LA[N];
        s.dbs[N-1].fill(0.0);
        for bi in 0..B { for j in 0..c { s.dbs[N-1][j] += s.deltas[N-1][bi*c+j]; } }
        for k in 0..c { bd[N-1][k] -= lr*s.dbs[N-1][k]; }
    }

    rayon::scope(|sc| {
        // Process layers N-1 down to 1: compute delta_prev, then spawn SGD(i)
        for i in (1..N).rev() {
            let (r,c) = (LA[i], LA[i+1]);

            // Compute delta_{i-1}
            if r > c {
                bwd_wt(s.deltas[i-1].as_mut_ptr(), ws[i].as_ptr(), s.deltas[i].as_ptr(), r,c,B);
            } else {
                let mut dt = vec![0f32; c*B]; let mut tmp = vec![0f32; r*B];
                const T: usize = 32;
                let mut a=0; while a<B { let am=(a+T).min(B); let mut b2=0; while b2<c {
                    let bm=(b2+T).min(c);
                    for aa in a..am { for bb in b2..bm { dt[bb*B+aa]=s.deltas[i][aa*c+bb]; } }
                    b2+=T; } a+=T; }
                sgemm(tmp.as_mut_ptr(), ws[i].as_ptr(), dt.as_ptr(), r,c,B);
                let mut a=0; while a<r { let am=(a+T).min(r); let mut b2=0; while b2<B {
                    let bm=(b2+T).min(B);
                    for aa in a..am { for bb in b2..bm { s.deltas[i-1][bb*r+aa]=tmp[aa*B+bb]; } }
                    b2+=T; } a+=T; }
            }

            // relu_mask + bias grad for layer i-1 (needs delta_{i-1} just computed)
            if i > 1 {
                let ci_prev = LA[i]; // c of layer i = r of layer i
                relu_mask_db(s.deltas[i-1].as_mut_ptr(), s.pres[i-1].as_ptr(),
                              s.dbs[i-1].as_mut_ptr(), B, ci_prev);
                let ci = LA[i];
                for k in 0..ci { bd[i-1][k] -= lr * s.dbs[i-1][k]; }
            }

            // Spawn SGD(i) — runs async while we compute delta_{i-1}
            let wout_ptr = wd[i].as_mut_ptr() as usize;
            let act_ptr  = s.acts[i].as_ptr() as usize;
            let delt_ptr = s.deltas[i].as_ptr() as usize;
            sc.spawn(move |_| {
                sgd_tn(wout_ptr as *mut f32, act_ptr as *const f32, delt_ptr as *const f32,
                       r, B, c, lr);
            });
        }

        // relu_mask for layer 0
        let c0 = LA[1];
        relu_mask_db(s.deltas[0].as_mut_ptr(), s.pres[0].as_ptr(),
                     s.dbs[0].as_mut_ptr(), B, c0);
        for k in 0..c0 { bd[0][k] -= lr * s.dbs[0][k]; }

        // SGD for layer 0 — done in main thread (scope waits for all spawned tasks)
        sgd_tn(wd[0].as_mut_ptr(), s.acts[0].as_ptr(), s.deltas[0].as_ptr(),
               LA[0], B, LA[1], lr);
    });
}
...
fn run_pipeline(x: &[f32], y: &[f32], lr: f32, steps: usize) -> (f64, f32) {
    ...
    for i in 0..N { w1[0][i].copy_from_slice(&w0[0][i]); }
    let mut bs = [
    ...
    for _ in 0..10 {
        ...
        let (wsr,wsw) = if ri<wi{(&lo[ri],&mut hi[0])}else{(&hi[0],&mut lo[wi])};
        let (blo,bhi) = bs.split_at_mut(wi.max(ri));
        let (bsr,bsw) = if ri<wi{(&blo[ri],&mut bhi[0])}else{(&bhi[0],&mut blo[wi])};
        do_fwd(&mut slots[0], wsr, bsr, x);
        unsafe { bwd_copy_sgd(&mut slots[0], wsr, wsw, bsw, y.as_ptr(), lr); }
        ri = wi;
    }
    ...
    for _step in 0..steps {
        let cur = ri & 1; let nxt = 1-cur; let wi = 1-ri;
        ...
        let xp = x.as_ptr() as usize; let yp = y.as_ptr() as usize;
        ...
        rayon::join(
            ...
            || unsafe {
                bwd_copy_sgd(
                    s_cur as *mut Slot,
                    ws_r as *const [Vec<f32>;N],
                    ws_w as *mut [Vec<f32>;N],
                    bs_w as *mut [Vec<f32>;N],
                    yp as *const f32, lr,
                );
            },
        );
...
fn main() {
    let x: Vec<f32> = (0..B*LA[0]).map(|i| (i%17) as f32*0.01-0.08).collect();
    ...
    for r in 0..RUNS { let (t,_) = run_pipeline(&x,&y,lr,STEPS); rp[r]=t; }
    rp.sort_by(|a,b|a.partial_cmp(b).unwrap());

    let pytorch = 2.440f64;
    println!("[final3 pipeline] p25={:.3} p50={:.3}  vs PyTorch {:+.1}%",
        rp[3], rp[7], (pytorch-rp[7])/pytorch*100.0);

...
fn par(m: usize, k: usize, n: usize) -> gemm::Parallelism {
    ...
    if 2*m*k*n < (1<<20) { gemm::Parallelism::None }
...
fn do_fwd(buf: &mut InfBuf, ws: &[Vec<f32>;N], bs: &[Vec<f32>;N], x: &[f32], b: usize) {
    ...
    for i in 0..N {
        let (r, c) = (LA[i], LA[i+1]);
        if i == N-1 {
            sgemm(buf.acts[N].as_mut_ptr(), buf.acts[i].as_ptr(), ws[i].as_ptr(), b, r, c);
            for bi in 0..b { for j in 0..c { buf.acts[N][bi*c+j] += bs[i][j]; } }
        } else {
            sgemm(buf.pres[i].as_mut_ptr(), buf.acts[i].as_ptr(), ws[i].as_ptr(), b, r, c);
            #[cfg(target_arch="x86_64")]
            unsafe { bias_relu(buf.pres[i].as_mut_ptr(), buf.acts[i+1].as_mut_ptr(), bs[i].as_ptr(), b, c); }
        }
    }
}
...
fn make_ws(rng: &mut u64) -> [Vec<f32>;N] {
    ...
    lcg_xavier(&mut ws[0], 512, 1024, rng);  lcg_xavier(&mut ws[1], 1024, 1024, rng);
    lcg_xavier(&mut ws[2], 1024, 512, rng);  lcg_xavier(&mut ws[3], 512, 256, rng);
...
fn run_inf(b: usize, runs: usize, steps: usize) -> (f64, f64) {
    ...
    // warmup
    for _ in 0..20 { do_fwd(&mut buf, &ws, &bs, &x, b); }

    let mut times = vec![0f64; runs];
    for r in 0..runs {
        let t0 = Instant::now();
        for _ in 0..steps { do_fwd(&mut buf, &ws, &bs, &x, b); }
...
fn run_inf_pipelined(b: usize, runs: usize, steps: usize) -> (f64, f64) {
    ...
    // warmup
    for _ in 0..20 { do_fwd(&mut bufs[0], &ws, &bs, &x, b); }
...
fn main() {
    ...
    for (b, pt) in [(64, pytorch_b64), (128, pytorch_b128), (256, pytorch_b256)] {
        ...
        println!("│  {:5} │  Volta sequential    │  {:.4}ms │  {:.4}ms │  {:+.1}% │",
            b, p25, p50, vs);

        let (p25p, p50p) = run_inf_pipelined(b, RUNS, STEPS);
        let vsp = (pt - p50p) / pt * 100.0;
        println!("│  {:5} │  Volta pipelined     │  {:.4}ms │  {:.4}ms │  {:+.1}% │",
            b, p25p, p50p, vsp);

        println!("│  {:5} │  PyTorch (ref)       │  --      │  {:.4}ms │  baseline│", b, pt);
        if b != 256 { println!("├─────────────────────────────────────────────────────────────┤"); }
    }
    println!("└─────────────────────────────────────────────────────────────┘");

    // also sweep PAR_T for optimal inference parallelism
    println!("\nPAR_T sweep (B=64, p50, {} runs × {} steps):", RUNS, STEPS);

...
fn par(m: usize, k: usize, n: usize) -> gemm::Parallelism {
    ...
    else if ops < (1<<25) { gemm::Parallelism::Rayon(2) }
...
fn compute_grads_pipelined_sgd(s: &mut Slot, ws: &[Vec<f32>;N], bs: &mut [Vec<f32>;N],
                                ws_out: &mut [Vec<f32>;N], y: &[f32], lr: f32) {
    let nt = B;
    let mut lacc = 0f32;
    for k in 0..nt { let d = s.acts[N][k]-y[k]; lacc+=d*d; s.deltas[N-1][k]=2.0*d/(nt as f32); }
    ...
    // Process layer N-1 first (no delta_prev needed, just SGD)
    {
        ...
        s.dbs[i].fill(0.0);
        for bi in 0..B { for j in 0..c { s.dbs[i][j] += s.deltas[i][bi*c+j]; } }
        for k in 0..c { bs[i][k] -= lr*s.dbs[i][k]; }
    }
    ...
    for i in (1..N).rev() {
            ...
            || unsafe {
                ...
                if r > c {
                    ...
                    let mut a=0; while a<B { let am=(a+T).min(B); let mut b2=0; while b2<c {
                        let bm=(b2+T).min(c);
                        for aa in a..am { for bb in b2..bm { dt[bb*B+aa]=dslice[aa*c+bb]; } }
                        b2+=T; } a+=T; }
                    ...
                    let mut a=0; while a<r { let am=(a+T).min(r); let mut b2=0; while b2<B {
                        ...
                        for aa in a..am { for bb in b2..bm { dpslice[bb*r+aa]=tmp[aa*B+bb]; } }
                        b2+=T; } a+=T; }
                }
            },
            || unsafe {
                // SGD for layer i (ws_out[i] already copied from ws[i])
                sgd_tn(wout_ptr as *mut f32, act_ptr as *const f32, delta_ptr as *const f32,
                       r, B, c, lr);
            },
        );
        ...
        if i-1 != N-1 && i > 1 {
            ...
            unsafe { relu_mask_db(s.deltas[i-1].as_mut_ptr(), s.pres[i-1].as_ptr(),
                                   s.dbs[i-1].as_mut_ptr(), B, ci); }
            for k in 0..ci { bs[i-1][k] -= lr*s.dbs[i-1][k]; }
        }
    }
    ...
    if N > 1 {
        let c0 = LA[1];
        unsafe { relu_mask_db(s.deltas[0].as_mut_ptr(), s.pres[0].as_ptr(),
                               s.dbs[0].as_mut_ptr(), B, c0); }
        for k in 0..c0 { bs[0][k] -= lr*s.dbs[0][k]; }
    }

    // SGD for layer 0 (last, no parallelism possible here)
    {
        let (r,c) = (LA[0], LA[1]);
        sgd_tn(ws_out[0].as_mut_ptr(), s.acts[0].as_ptr(), s.deltas[0].as_ptr(), r,B,c,lr);
    }
}
...
fn run_pipeline_layerpipe(x: &[f32], y: &[f32], lr: f32, steps: usize) -> (f64, f32) {
    ...
    for _ in 0..10 {
        ...
        for i in 0..N { wsw[i].copy_from_slice(&wsr[i]); bsw[i].copy_from_slice(&bsr[i]); }
    ...
    for _step in 0..steps {
        ...
        rayon::join(
            ...
            || unsafe {
                ...
                rayon::join(
                    || { for i in 0..N { (*ws_dst)[i].copy_from_slice(&(*ws_src)[i]); } },
                    || {
                        ...
                        let mut lacc = 0f32;
                        for k in 0..nt {
                            let d = s.acts[N][k] - (*ysl.as_ptr().add(k)); lacc+=d*d;
                        ...
                        let cl = LA[N];
                        s.dbs[N-1].fill(0.0);
                        for bi in 0..B { for j in 0..cl { s.dbs[N-1][j] += s.deltas[N-1][bi*cl+j]; } }
                        for k in 0..cl { (*bs_dst)[N-1][k] -= lr * s.dbs[N-1][k]; }
                        // backward layers
                        for i in (1..N).rev() {
                            let (r,c) = (LA[i], LA[i+1]);
                            if r > c {
                                bwd_wt(s.deltas[i-1].as_mut_ptr(), ws_src[i].as_ptr(), s.deltas[i].as_ptr(), r,c,B);
                            } else {
                                let mut dt = vec![0f32; c*B]; let mut tmp = vec![0f32; r*B];
                                const T: usize = 32;
                                let mut a=0; while a<B { let am=(a+T).min(B); let mut b2=0; while b2<c {
                            ...
                            if i > 1 {
                                ...
                                relu_mask_db(s.deltas[i-1].as_mut_ptr(), s.pres[i-1].as_ptr(),
                                             s.dbs[i-1].as_mut_ptr(), B, ci);
                                for k in 0..ci { (*bs_dst)[i-1][k] -= lr * s.dbs[i-1][k]; }
                            }
                        }
                        let c0 = LA[1];
                        relu_mask_db(s.deltas[0].as_mut_ptr(), s.pres[0].as_ptr(),
                                     s.dbs[0].as_mut_ptr(), B, c0);
                        for k in 0..c0 { (*bs_dst)[0][k] -= lr * s.dbs[0][k]; }
                    },
                );
                // Now ws_dst is copied AND grads computed — apply SGD
                for i in 0..N {
                    let (r,c) = (LA[i], LA[i+1]);
                    sgd_tn((*ws_dst)[i].as_mut_ptr(), s.acts[i].as_ptr(), s.deltas[i].as_ptr(), r,B,c,lr);
                }
            },
        );
...
fn main() {
    ...
    println!("measuring layer-pipelined bwd+sgd pipeline ({} runs)...", RUNS);
    for r in 0..RUNS { let (t,_) = run_pipeline_layerpipe(&x,&y,lr,STEPS); rp[r]=t; }
    rp.sort_by(|a,b|a.partial_cmp(b).unwrap());

    let pytorch = 2.440f64;
    let p25 = rp[3]; let p50 = rp[7];
    println!("[layer-pipe] p25={:.3} ms  p50={:.3} ms  vs PyTorch: {:+.1}%",
        p25, p50, (pytorch-p50)/pytorch*100.0);

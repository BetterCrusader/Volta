...
fn par(m: usize, k: usize, n: usize) -> gemm::Parallelism {
    ...
    else if ops < (1<<25) { gemm::Parallelism::Rayon(4) }
...
fn bwd_wt(dp: *mut f32, w: *const f32, d: *const f32, r: usize, c: usize, batch: usize) {
    unsafe { gemm::gemm(batch,r,c, dp,1,r as isize,false, d,1,c as isize, w,c as isize,1, 0f32,1f32,false,false,false, par(batch,c,r)); }
}
...
unsafe fn bias_relu(pre: *mut f32, act: *mut f32, bias: *const f32, rows: usize, cols: usize) {
    ...
    for bi in 0..rows { let base=bi*cols; let mut j=0usize;
        while j+16<=cols {
            let b=_mm512_loadu_ps(bias.add(j));
            let p=_mm512_add_ps(_mm512_loadu_ps(pre.add(base+j)),b);
            _mm512_storeu_ps(pre.add(base+j),p);
            _mm512_storeu_ps(act.add(base+j),_mm512_max_ps(p,zero)); j+=16;
        }
        if j<cols { let rem=(cols-j)as u16; let mask=(1u16<<rem)-1;
...
unsafe fn relu_mask_db(delta: *mut f32, pre: *const f32, db: *mut f32, rows: usize, cols: usize) {
    ...
    for bi in 0..rows { let base=bi*cols; let mut j=0usize;
        while j+16<=cols {
            let p=_mm512_loadu_ps(pre.add(base+j)); let d=_mm512_loadu_ps(delta.add(base+j));
            let mask=_mm512_cmp_ps_mask(p,zero,_CMP_GT_OQ);
            let d2=_mm512_maskz_mov_ps(mask,d);
            _mm512_storeu_ps(delta.add(base+j),d2);
            _mm512_storeu_ps(db.add(j),_mm512_add_ps(_mm512_loadu_ps(db.add(j)),d2)); j+=16;
        }
        if j<cols { let rem=(cols-j)as u16; let km=(1u16<<rem)-1;
            ...
            _mm512_mask_storeu_ps(db.add(j),km,_mm512_add_ps(_mm512_maskz_loadu_ps(km,db.add(j)),d2));
        }
    }
}
...
impl Weights {
    fn new() -> Self {
        ...
        lcg_xavier(&mut ws0[0],512,1024,&mut rng); lcg_xavier(&mut ws0[1],1024,1024,&mut rng);
        lcg_xavier(&mut ws0[2],1024,512,&mut rng);  lcg_xavier(&mut ws0[3],512,256,&mut rng);
    ...
    fn write(&self) -> usize { 1 - self.read }
}
...
fn do_fwd(slot: &mut Slot, ws: &[Vec<f32>;N], bs: &[Vec<f32>;N], x: &[f32]) {
    ...
    for i in 0..N {
        ...
        if i==N-1 {
            sgemm(slot.acts[N].as_mut_ptr(), slot.acts[i].as_ptr(), ws[i].as_ptr(), B,r,c);
            for bi in 0..B { for j in 0..c { slot.acts[N][bi*c+j]+=bs[i][j]; } }
        } else {
            sgemm(slot.pres[i].as_mut_ptr(), slot.acts[i].as_ptr(), ws[i].as_ptr(), B,r,c);
            #[cfg(target_arch="x86_64")]
            unsafe { bias_relu(slot.pres[i].as_mut_ptr(),slot.acts[i+1].as_mut_ptr(),bs[i].as_ptr(),B,c); }
        }
    }
}

fn do_bwd_sgd(slot: &mut Slot, ws_write: &mut [Vec<f32>;N], bs: &mut [Vec<f32>;N],
              ws_read: &[Vec<f32>;N], y: &[f32], lr: f32) {
    // copy read→write before updating
    for i in 0..N { ws_write[i].copy_from_slice(&ws_read[i]); }

    let nt=B*1; let mut lacc=0f32;
    for k in 0..nt { let d=slot.acts[N][k]-y[k]; lacc+=d*d; slot.deltas[N-1][k]=2.0*d/(nt as f32); }
    ...
    for i in (0..N).rev() {
        ...
        if !is_last {
            #[cfg(target_arch="x86_64")]
            unsafe { relu_mask_db(slot.deltas[i].as_mut_ptr(),slot.pres[i].as_ptr(),slot.dbs[i].as_mut_ptr(),B,c); }
            for k in 0..c { bs[i][k]-=lr*slot.dbs[i][k]; }
        }
        if i>0 {
            if r>c { bwd_wt(slot.deltas[i-1].as_mut_ptr(),ws_write[i].as_ptr(),slot.deltas[i].as_ptr(),r,c,B); }
            else {
                // transpose + sgemm + transpose for r<=c
                let mut dt=vec![0f32;c*B]; let mut tmp=vec![0f32;r*B];
                const T:usize=32;
                let mut ii=0; while ii<B { let im=(ii+T).min(B); let mut jj=0; while jj<c {
                    let jm=(jj+T).min(c);
                    unsafe{for a in ii..im{for b in jj..jm{*dt.as_mut_ptr().add(b*B+a)=slot.deltas[i][a*c+b];}}}
                    jj+=T;} ii+=T;}
                sgemm(tmp.as_mut_ptr(),ws_write[i].as_ptr(),dt.as_ptr(),r,c,B);
                let mut ii=0; while ii<r { let im=(ii+T).min(r); let mut jj=0; while jj<B {
                    let jm=(jj+T).min(B);
                    unsafe{for a in ii..im{for b in jj..jm{slot.deltas[i-1][b*r+a]=*tmp.as_ptr().add(a*B+b);}}}
                    jj+=T;} ii+=T;}
            }
        }
        sgd_tn(ws_write[i].as_mut_ptr(),slot.acts[i].as_ptr(),slot.deltas[i].as_ptr(),r,B,c,lr);
        if is_last {
            slot.dbs[i].fill(0.0);
            for bi in 0..B { for j in 0..c { slot.dbs[i][j]+=slot.deltas[i][bi*c+j]; } }
            for k in 0..c { bs[i][k]-=lr*slot.dbs[i][k]; }
        }
    }
}
...
fn bench_pipeline(x: &[f32], y: &[f32], lr: f32, steps: usize) -> (f64, f32) {
    ...
    for _ in 0..10 {
        let r = wts.read; let w = wts.write();
        ...
        unsafe { do_bwd_sgd(&mut slots[0], &mut *ws_w, &mut *std::ptr::addr_of_mut!(wts.bs), &*ws_r, y, lr); }
        let _ = bs_ptr;
        wts.read = w;
    }
...
fn bench_baseline(x: &[f32], y: &[f32], lr: f32, steps: usize) -> (f64, f32) {
    ...
    for _ in 0..10 {
        ...
        let ws_r = &wts.ws[r] as *const [Vec<f32>;N];
        let ws_w = &mut wts.ws[w] as *mut [Vec<f32>;N];
        unsafe { do_bwd_sgd(&mut slot, &mut *ws_w, &mut wts.bs, &*ws_r, y, lr); }
        wts.read=w;
    }
    let t0=Instant::now();
    for _ in 0..steps {
...
fn main() {
    ...
    println!("[baseline] median={:.3} ms  all={:?}", results_b[3], results_b.map(|x|format!("{:.3}",x)));
    ...
    for r in 0..7 {
        ...
        results_p[r]=t; cs+=loss;
    }
    results_p.sort_by(|a,b|a.partial_cmp(b).unwrap());
    println!("[pipeline] median={:.3} ms  all={:?}", results_p[3], results_p.map(|x|format!("{:.3}",x)));
    println!("  checksum={:.6} (differs from v3 due to stale W — expected)", cs);

...
fn main() {
    ...
    #[cfg(windows)] pin_current_thread(0);

    // Build rayon pool with thread pinning
    rayon::ThreadPoolBuilder::new()
        .num_threads(12)
        .start_handler(|i| {
            #[cfg(windows)] pin_current_thread(i + 1); // cores 1..12
        })
        .build_global()
        .ok();
    ...
    for r in 0..RUNS { let (t,_) = run_sequential(&x,&y,lr,STEPS); rv[r]=t; }
    ...
    println!("[sequential] p25={:.3} p50={:.3}  vs PyTorch {:+.1}%", rv[3], rv[7], (pytorch-rv[7])/pytorch*100.0);
    println!("[pipeline  ] p25={:.3} p50={:.3}  vs PyTorch {:+.1}%", rp[3], rp[7], (pytorch-rp[7])/pytorch*100.0);

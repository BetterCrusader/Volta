...
fn par(m: usize, k: usize, n: usize) -> gemm::Parallelism {
    let t = PAR_T.load(std::sync::atomic::Ordering::Relaxed);
    let ops = 2*m*k*n;
    if ops < (1<<20) { gemm::Parallelism::None }
    else { gemm::Parallelism::Rayon(t) }
}
...
// faer backward for r<=c layers: delta_prev[B×r] = delta[B×c] @ W[r×c]^T
use faer::linalg::matmul::matmul as fmatmul;
use faer::Accum as FAccum;
...
fn bwd_wt_faer(dp: *mut f32, w: *const f32, d: *const f32, r: usize, c: usize, batch: usize) {
    unsafe {
        fmatmul(as_mat_mut(dp,batch,r), FAccum::Replace,
...
// v3-style: sequential, single buffer, no overhead
fn run_v3(x: &[f32], y: &[f32], lr: f32, steps: usize) -> (f64, f32) {
    let mut ws = make_ws();
...
fn run_pipeline(x: &[f32], y: &[f32], lr: f32, steps: usize) -> (f64, f32) {
    ...
    for _ in 0..10 {
        ...
        for i in 0..N { wsw[i].copy_from_slice(&wsr[i]); }
        for i in 0..N { bsw[i].copy_from_slice(&bsr[i]); }
...
fn main() {
    ...
    let mut rv = [0f64;RUNS]; let mut rp = [0f64;RUNS];
    println!("\nFinal measurement {} runs × {} steps...", RUNS, STEPS);
    for r in 0..RUNS { let (t,_) = run_v3(&x,&y,lr,STEPS); rv[r]=t; }
    ...
    println!("├─────────────────────────────────────────────────────┤");
    println!("│  PyTorch (ref)    │  --     │  {:.3} ms │  baseline    │", pytorch);
    println!("│  v3 sequential    │  {:.3} ms │  {:.3} ms │  {:+.1}%      │",
        rv[3], rv[7], (pytorch-rv[7])/pytorch*100.0);
    println!("│  pipeline 3-way   │  {:.3} ms │  {:.3} ms │  {:+.1}%      │",
        rp[3], rp[7], (pytorch-rp[7])/pytorch*100.0);
    println!("└─────────────────────────────────────────────────────┘");
    println!("\nPipeline p50 all: {:?}", rp.map(|v|format!("{:.3}",v)));
    println!("v3       p50 all: {:?}", rv.map(|v|format!("{:.3}",v)));

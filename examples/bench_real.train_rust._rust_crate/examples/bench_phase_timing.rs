...
fn par(m: usize, k: usize, n: usize) -> gemm::Parallelism {
    ...
    else if ops < (1<<25) { gemm::Parallelism::Rayon(0) }
...
fn bench_us(label: &str, mut f: impl FnMut()) -> f64 {
    ...
    for r in 0..7 {
        let t0 = Instant::now();
        for _ in 0..100 { f(); }
        res[r] = t0.elapsed().as_nanos() as f64 / 100.0 / 1000.0;
    }
...
fn main() {
    ...
    let t_copy = bench_us("copy_ws (~8.5MB)", || {
        for i in 0..N { ws2[i].copy_from_slice(&ws[i]); }
    });
    ...
    let t_fwd_grads_par = bench_us("fwd || grads (rayon::join)", || {
        ...
        rayon::join(
            || unsafe { do_fwd(&mut *(sp as *mut Slot), &*(wsp as *const [Vec<f32>;N]), &*(bsp as *const [Vec<f32>;N]), std::slice::from_raw_parts(xp as *const f32, B*LA[0])); },
            || unsafe { compute_grads(&mut *(sp as *mut Slot), &*(wsp as *const [Vec<f32>;N]), &mut *(bs2p as *mut [Vec<f32>;N]), std::slice::from_raw_parts(yp as *const f32, B*LA[N]), lr); },
        );
    });
    ...
    let t_3way = bench_us("fwd || (copy_ws || grads)", || {
        ...
        let yp = y.as_ptr() as usize;
        rayon::join(
            || unsafe { do_fwd(&mut *(sp2 as *mut Slot), &*(wsp as *const [Vec<f32>;N]), &*(bsp as *const [Vec<f32>;N]), std::slice::from_raw_parts(xp as *const f32, B*LA[0])); },
            || unsafe {
                rayon::join(
                    || { let ws_src = &*(wsp as *const [Vec<f32>;N]); let ws_dst = &mut *(ws2p as *mut [Vec<f32>;N]); for i in 0..N { (*ws_dst)[i].copy_from_slice(&(*ws_src)[i]); } },
                    || { compute_grads(&mut *(sp as *mut Slot), &*(wsp as *const [Vec<f32>;N]), &mut *(bs2p as *mut [Vec<f32>;N]), std::slice::from_raw_parts(yp as *const f32, B*LA[N]), lr); },
                );
            },
        );
    });

    let _ = (rng, t_fwd, t_grads, t_sgd, t_copy, t_fwd_grads_seq, t_fwd_grads_par, t_3way);
    ...
    println!("  fwd||grads:    {:.1} us  (efficiency {:.0}%)", t_fwd_grads_par,
        (t_fwd+t_grads)/2.0/t_fwd_grads_par*100.0);
    println!("  fwd||(copy||grads): {:.1} us  vs pipeline result ~2110 us", t_3way);
    println!("  theoretical min (fwd||grads+sgd): {:.1} us", t_fwd.max(t_grads+t_sgd));
